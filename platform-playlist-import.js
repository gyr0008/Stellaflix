...
// 平台歌单导入服务 — 自 Mineradio-LX-Music (GPL-3.0) services/platform-playlist-import.js
// 移植，基准 commit 34b87f9（2026-09-20 克隆）。移植改动：
// 1) 去除对 ./lx-search 的硬依赖：跨平台匹配桥改为 setSearchImplementation()
//    注入（Stellaflix server.js 注入 netease handleSearch 适配器，见 server.js 路由区）。
// 2) Spotify 缓存目录改读 STELLAFLIX_SPOTIFY_PLAYLIST_CACHE_DIR / Stellaflix 目录。
// 其余解析逻辑（tx/wy/kw/kg/kgc/mg/sp/qs/am 九平台 + 短链展开 + 专辑导入）保持原样。
...
// —— 跨平台搜索桥（原 lxSearch.searchAll）——
// 未注入时返回空结果：歌曲标记 needsCrossPlatformMatch，由前端
// custom-source 聚合解析按 name+singer 现场匹配，导入链路仍可用。
let searchBridge = null;
function setSearchImplementation(fn) {
  if (typeof fn === 'function') searchBridge = fn;
}
async function bridgeSearchAll(query, options) {
  if (typeof searchBridge !== 'function') return { ok: true, songs: [], failures: [{ source: 'bridge', reason: 'NOT_INJECTED' }] };
  try {
    const result = await searchBridge(String(query || ''), options || {});
    return result && typeof result === 'object' && Array.isArray(result.songs)
      ? result
      : { ok: true, songs: [], failures: [] };
  } catch (error) {
    return { ok: true, songs: [], failures: [{ source: 'bridge', reason: (error && error.message) || 'BRIDGE_FAILED' }] };
  }
}
...
const SPOTIFY_PLAYLIST_CACHE_DIR = process.env.STELLAFLIX_SPOTIFY_PLAYLIST_CACHE_DIR || path.join(
...
async function matchReferenceSongsForPlayback(rows, metaKey) {
  ...
  async function worker() {
    while (cursor < rows.length) {
      ...
      try {
        const found = await bridgeSearchAll([row.name, platformMatchText(row.singer)].filter(Boolean).join(' '), {
...
async function importSpotify(id, input, context = {}) {
  ...
  const found = await bridgeSearchAll([songName, artistHint].filter(Boolean).join(' '), { sources:'tx,wy,kw,kg,mg', limit:8 });
...
async function importQishui(id, input, context = {}) {
  ...
  async function worker() {
    while (cursor < rows.length) {
      ...
      const row = rows[index];
      try {
        const found = await bridgeSearchAll(`${row.name} ${row.singer}`, { sources:'tx,wy,kw,kg,mg', limit:6 });
...
module.exports = { importPlaylist, setFetchImplementation, setSearchImplementation, detect, expandShareLinkDetailed, extractMiguPlaylistId, extractMiguPlaylistIdentity, extractKugouPlaylistIdentity, extractKugouNormalIdentity, extractKugouConceptIdentity };
