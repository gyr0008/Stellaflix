...
function normalizePlaybackProvider(provider) {
  ...
  if (provider === 'lx-custom-source' || provider === 'custom-source' || provider === 'custom') return 'lx-custom-source';
  return 'netease';
}
...
function bindAudioOutputMirrorEvents(media) {
  if (!media || media._stellaflixAudioMirrorBound) return;
  media._stellaflixAudioMirrorBound = true;
...
function syncAudioOutputMirrors(reason) {
  ...
  ids.forEach(function (id) {
    ...
    if (mirror._stellaflixSinkId !== id || !mirror._stellaflixSinkReady) {
      mirror._stellaflixSinkId = id;
      if (!mirror._stellaflixSinkBusy) {
        mirror._stellaflixSinkBusy = true;
        Promise.resolve(applyAudioOutputMirrorSink(mirror, id)).then(function (ok) {
          mirror._stellaflixSinkBusy = false;
          mirror._stellaflixSinkReady = !!ok;
    ...
    if (!mirror._stellaflixSinkReady) return;
