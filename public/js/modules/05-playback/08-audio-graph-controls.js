...
function replaceAudioElementForGraphRecovery(reason, opts) {
  ...
  var queueItemKey = oldAudio.__stellaflixQueueItemKey;
  ...
  audio.__stellaflixQueueItemKey = queueItemKey;
...
function resetPlaybackAudioGraphForSourceSwitch(reason) {
  if (!audio) return;
  var preparedGraph = audio.__stellaflixPreparedAudioGraph;
  var previousSourceMedia = audioSourceMedia;
  var sourceUsesCapture = !!(source && source.__stellaflixUsesCapture);
  ...
  if (
    ...
    audio.__stellaflixMediaSourceBound = true;
...
function initAudio() {
  ...
  var sourceUsesCapture = !!(keepSource && source.__stellaflixUsesCapture);
  disconnectAudioGraphNodes(keepSource);
  if (!source) {
    var forceCapture = !!audio.__stellaflixForceCaptureSource;
    ...
    if (!forceCapture && !audio.__stellaflixMediaSourceBound && audioCtx.createMediaElementSource) {
      try {
    ...
    if (!forceCapture && !mediaSource && audio.__stellaflixMediaSourceBound) {
    ...
    source.__stellaflixUsesCapture = !mediaSource;
    audioSourceMedia = audio;
    ...
    if (mediaSource) audio.__stellaflixMediaSourceBound = true;
...
function rebuildPlaybackGraphWithCapture(reason) {
  ...
  if (audio.__stellaflixMediaSourceBound) {
    if (!source || source.__stellaflixUsesCapture || audioSourceMedia !== audio) return false;
...
function schedulePlaybackAnalyserRecovery(reason) {
  ...
  [720, 1600, 2800].forEach(function (delay) {
    setTimeout(function () {
      ...
      if (source && !source.__stellaflixUsesCapture && audio.captureStream) {
