var CUEFIELD_AUTOMIX_STORE_KEY = 'stellaflix-cuefield-automix-v1';
...
function claimCuefieldPreparedAudioForPlayback(media) {
  ...
  cancelCuefieldMediaFade();
  if (media.__stellaflixPreparedAudioGraph) media.__stellaflixPreparedAudioGraph.adopted = true;
  if (media === cuefieldAutoMixPreparedAudio) cuefieldAutoMixPreparedAudio = null;
  return true;
}

function disposeCuefieldPreparedAudioGraph(media) {
  var graph = media && media.__stellaflixPreparedAudioGraph;
  ...
  try { delete media.__stellaflixPreparedAudioGraph; } catch (_) { }
}

function stopCuefieldPreparedAudio(media) {
  media = media || cuefieldAutoMixPreparedAudio;
  if (!media) return;
  // Once the preloaded B deck has become Stellaflix's active deck it no longer
...
function cuefieldCreatePreparedAudioGraph(media) {
  if (!media || media.__stellaflixPreparedAudioGraph) return media && media.__stellaflixPreparedAudioGraph || null;
  var graph = null;
  try {
    ...
    media.__stellaflixMediaSourceBound = true;
    ...
    media.__stellaflixPreparedAudioGraph = graph;
    return graph;
  } catch (error) {
    if (graph) {
      [graph.source, graph.analyser, graph.beatAnalyser, graph.gainNode].forEach(function (node) {
    ...
    if (media && media.__stellaflixMediaSourceBound) media.__stellaflixPreparedGraphFailed = true;
    try { delete media.__stellaflixPreparedAudioGraph; } catch (_) { }
...
function prepareCuefieldPendingAudio(pending) {
  ...
  if (media.__stellaflixPreparedGraphFailed) {
...
async function runCuefieldTimeline(pending, nextMedia, context) {
  ...
  pending.executionFallback = nextMedia.__stellaflixPreparedAudioGraph ? 'shared-context-gain' : 'direct-volume-fallback';
...
function noteCuefieldAutoMixOutgoingEnded(media, token, index) {
  if (!media) return false;
  media.__stellaflixCuefieldEndedDeferredToken = Number(token);
  media.__stellaflixCuefieldEndedDeferredIndex = Number(index);
  return true;
}
...
function recoverCuefieldAutoMixEndedOutgoing(pending, context, reason) {
  ...
  if (outgoing.__stellaflixCuefieldEndedRecoveryToken === token) return true;
  outgoing.__stellaflixCuefieldEndedRecoveryToken = token;
