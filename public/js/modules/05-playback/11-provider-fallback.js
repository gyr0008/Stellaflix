...
function playbackProviderLabel(song) {
  var provider = (song && (song.resolvedPlaybackProvider || song.playbackProvider || song.audioProvider || song.providerResolved)) || songProviderKey(song);
  if (provider === 'lx-custom-source') return song && (song.resolvedSourceLabel || song.sourceLabel || '第三方音源');
...
function settleSourceFallbackTerminal(idx, token, message, opts) {
  ...
  if (audio) {
    try {
      ...
      audio.__stellaflixQueueItemKey = '';
      audio.__stellaflixTrackSwitchToken = 0;
      audio.load();
    } catch (e) { }
  }
  playing = false;
...
async function tryAutoPlaybackFallback(song, data, idx, token, opts) {
  ...
  if (integration && typeof integration.fallbackOnPlaybackFailure === 'function') {
    try {
      var rescue = await integration.fallbackOnPlaybackFailure(song, data, idx, token, opts);
      if (rescue && rescue.result === true) return true;
      if (rescue && rescue.result === false) return false;
    } catch (customFallbackErr) {
      console.warn('[CustomSource] fallback hook error', customFallbackErr && customFallbackErr.message || customFallbackErr);
    }
  }
