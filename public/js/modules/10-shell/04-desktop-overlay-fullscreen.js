...
function updateDesktopModeControl(status) {
  ...
  if (softwareLockButton) {
    ...
    softwareLockButton.title = softwareLocked ? '恢复 Stellaflix 操作' : '暂时把操作交给 Windows 桌面';
  }
...
function currentDesktopLyricSnapshot() {
  ...
  return { text: normalizeDesktopLyricText(currentDesktopSongMeta().title || 'Stellaflix'), progress: 0, progressSpan: 4.8 };
}
...
function updateDesktopWallpaperRuntimeControls(status) {
  ...
  if (toggle) {
    ...
    toggle.title = !supported
      ? '当前系统不支持完整桌面模式'
      : (attaching ? '正在切换完整桌面模式' : '把完整 Stellaflix 放到 Windows 桌面；右上角控制器可显示或隐藏桌面图标，Esc 退出');
  }
...
function revealDesktopWallpaperUiOnActivation(enabled, interactive) {
  ...
  // Full desktop mode is the complete Stellaflix workspace. Do not inherit the
