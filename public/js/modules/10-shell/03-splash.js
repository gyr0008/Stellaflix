...
function initStellaflixSplashWebgl(canvas) {
  var gl = null;
  try {
...
function drawStellaflixSplashWebgl(elapsed) {
  var gl = splashGl;
...
(function initStellaflixSplashCanvas() {
  splashCanvas = document.getElementById('splash-canvas');
  if (!splashCanvas) return;
  if (!reduceSplashMotion && initStellaflixSplashWebgl(splashCanvas)) {
    splashCtx = null;
  } else {
...
function playStellaflixIntroSound() {
  ...
  try {
    ...
    if (ctx.state === 'suspended' && ctx.resume) {
      ctx.resume().then(function () {
        if (!splashSoundPlayed) playStellaflixIntroSound();
      }).catch(function () { });
