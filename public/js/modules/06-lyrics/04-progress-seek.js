...
function bindPlaybackProgressEvents(audioEl) {
  if (!audioEl || audioEl._stellaflixProgressBound) return;
  audioEl._stellaflixProgressBound = true;
  ...
  ['play', 'playing', 'pause', 'ended', 'emptied', 'abort', 'error'].forEach(function (name) {
    audioEl.addEventListener(name, function () {
      ...
      if (Number(audioEl.__stellaflixTrackSwitchToken) !== Number(trackSwitchToken)) return;
      if (
        name !== 'emptied'
    ...
    audioEl.addEventListener(name, function () {
      ...
      if (typeof schedulePlaybackStallRecovery === 'function') {
        schedulePlaybackStallRecovery(name, {
          ...
          ownerQueueItemKey: String(audioEl.__stellaflixQueueItemKey || '')
        });
      }
    });
  });
