...
/* ---------- 状态切换器（启动默认空间）：音乐空间 ⇄ 影视空间 ---------- */
/* 对齐 Stellaflix-Extended 的 t-startSpace 设计：点击切换「下次启动默认进入的空间」，
   同时立即切到该空间（与参考项目行为一致）。视觉为双层文字层叠淡入淡出。 */
function getStartSpacePreference() {
  var e = window.StellaflixVideo;
  if (e && e.state && typeof e.state.getStartSpace === 'function') return e.state.getStartSpace();
  try {
    var t = localStorage.getItem('stellaflix-start-space');
...
function setStartSpacePreference(mode) {
  var e = window.StellaflixVideo;
  if (e && e.state && typeof e.state.setStartSpace === 'function') {
    e.state.setStartSpace(mode);
    return;
  }
  try { localStorage.setItem('stellaflix-start-space', mode); } catch (_) {}
}
function updateStartSpaceUI() {
  var el = document.getElementById('t-startSpace');
  if (!el) return;
  var mode = getStartSpacePreference();
  el.classList.toggle('start-space-video', mode === 'video');
  el.setAttribute('aria-checked', mode === 'video' ? 'true' : 'false');
  el.setAttribute('data-start-space', mode);
}
function toggleStartSpace() {
  ...
  updateStartSpaceUI();
  // 仅改「启动默认空间」，不立即切换当前运行态（与用户确认的行为：点击只改下次启动默认，当前音乐/影视态不动）
  if (typeof showToast === 'function') {
    showToast(next === 'video' ? '下次启动将进入影视空间' : '下次启动将进入音乐空间');
  }
}
window.toggleStartSpace = toggleStartSpace;
window.updateStartSpaceUI = updateStartSpaceUI;
