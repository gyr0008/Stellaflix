function markAppPerf(name) {
  try {
    ...
    if (performance && performance.mark) performance.mark('stellaflix:' + name);
    if (appPerfMarks.length <= 16) console.debug('[StellaflixPerf]', name, Math.round(value) + 'ms');
  } catch (e) { }
}
  ...
  try {
    ...
    var observer = new PerformanceObserver(function (list) {
      list.getEntries().forEach(function (entry) {
        ...
        console.debug('[StellaflixPerf] longtask', Math.round(entry.startTime) + 'ms', Math.round(entry.duration) + 'ms');
      });
    });
