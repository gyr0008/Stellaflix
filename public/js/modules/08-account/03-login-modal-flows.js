...
var LOGIN_WORKFLOW_CONNECTION_STORE_KEY = 'stellaflix-login-workflow-connections-v1';
...
function updateLoginProviderUi() {
  ...
  if (isSpotify) {
    ...
    if (desc) desc.innerHTML = canOpenSpotifyOAuth
      ...
      : '当前环境不支持桌面授权桥；请在 Stellaflix 桌面版中连接 Spotify。';
    if (shell) {
...
async function openSpotifyWebLogin() {
  ...
  if (!api || !api.isDesktop || typeof api.openSpotifyMusicLogin !== 'function') {
    updateLoginProviderUi();
    if (statusEl) { statusEl.textContent = '当前环境不支持 Spotify 本地授权桥，请使用 Stellaflix 桌面版。'; statusEl.className = 'fail'; }
    return;
  }
