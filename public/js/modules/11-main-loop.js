...
window.__stellaflixRenderPerf = renderPerfState;
if (window.__stellaflixPerf && typeof window.__stellaflixPerf.registerRenderState === 'function') {
  window.__stellaflixPerf.registerRenderState(renderPerfState);
} else {
  window.__stellaflixPerf = renderPerfState;
}
...
function shouldSkipAdaptiveRenderFrame(now) {
  ...
  if (cadence) {
    ...
    if (cadence.divisor > 1 && (renderPerfState.adaptiveCadenceTick - 1) % cadence.divisor !== 0) {
      ...
      if (window.__stellaflixPerf && window.__stellaflixPerf.count) window.__stellaflixPerf.count('frame.skipped');
      return true;
    }
...
function animate() {
  ...
  if (isMainSceneCoveredBySplash()) {
    var splashWorkshopActive = window.StellaflixSonicWorkshop && StellaflixSonicWorkshop.isActive(fx);
    if (window.StellaflixSonicWorkshop) {
      StellaflixSonicWorkshop.update(dt, {
        scene: scene,
        fx: fx,
  ...
  var sonicPresetActiveEarly = window.StellaflixSonicTopography && StellaflixSonicTopography.isActive(fx);
  var sonicWorkshopActiveEarly = window.StellaflixSonicWorkshop && StellaflixSonicWorkshop.isActive(fx);
  ...
  var workshopPresetActive = window.StellaflixSonicWorkshop && StellaflixSonicWorkshop.isActive(fx);
  ...
  if (window.StellaflixSonicTopography) {
    StellaflixSonicTopography.update(dt, {
      scene: scene,
      fx: fx,
      time: uniforms.uTime.value,
