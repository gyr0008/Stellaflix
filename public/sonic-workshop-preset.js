...
(function (global) {
  var INDEX = 8;
  var BRIDGE_SRC = 'vendor/sonic-workshop/stellaflix-bridge.html';
  ...
  function workshopCustomThemeForColor(hex) {
    ...
    return {
      name: 'Stellaflix Custom',
      id: 'stellaflix-custom',
      __primaryColor: hex,
  ...
  function workshopCustomThemeForRegions(regions) {
    ...
    return {
      name: 'Stellaflix Region Palette',
      id: 'stellaflix-custom',
  ...
  function cloneWorkshopTheme(theme) {
    ...
    var out = {
      name: theme.name || 'Stellaflix Region Palette',
      id: theme.id || 'stellaflix-custom',
  ...
  function mixWorkshopTheme(from, to, t) {
    ...
    t = clamp01(t);
    var out = {
      name: to.name || from.name || 'Stellaflix Region Palette',
      id: to.id || from.id || 'stellaflix-custom',
  ...
  function applyWorkshopThemeTransition(props) {
    if (!props || !props.stellaflixCustomTheme) return props;
    var target = cloneWorkshopTheme(props.stellaflixCustomTheme);
    state.displayedTheme = target;
    state.themeTransition = null;
    props.stellaflixCustomTheme = target;
  ...
  function postMessage(type, payload) {
    ...
    if (!frame) return;
    try {
      if (type === 'stellaflix-sonic-workshop-audio' && typeof frame.__stellaflixApplyAudio === 'function') {
        frame.__stellaflixApplyAudio(payload.samples);
        return;
      }
      if (type === 'stellaflix-sonic-workshop-media' && typeof frame.__stellaflixApplyMedia === 'function') {
        frame.__stellaflixApplyMedia(payload.media);
        return;
      }
      if (type === 'stellaflix-sonic-workshop-properties' && typeof frame.__stellaflixApplyProperties === 'function') {
        frame.__stellaflixApplyProperties(payload.properties);
        return;
      }
    } catch (e) {}
  ...
  function deriveProperties(fx) {
    ...
    props.theme = 'stellaflix-custom';
    props.stellaflixCustomTheme = workshopCustomThemeForRegions(regions);
    props.schemecolor = hexToSchemeColor(colorHex);
    props.__stellaflixColorHex = colorHex;
    props.__stellaflixPaletteHexes = paletteHexes.join(',');
    props.__stellaflixNearestTheme = fx.sonicWorkshopColorMode === 'custom' ? customTheme : workshopThemeForColor(colorHex);
    return props;
  }
  ...
  function pushProperties(force) {
    ...
    postMessage('stellaflix-sonic-workshop-properties', { properties: props });
  }
  ...
  function pushMedia(force) {
    ...
    state.media = media;
    postMessage('stellaflix-sonic-workshop-media', { media: media });
  }
  ...
  function pushAudio(force, audioFrame) {
    ...
    postMessage('stellaflix-sonic-workshop-audio', { samples: buildAudioSamples(audioFrame || {}) });
  }
  ...
  global.addEventListener && global.addEventListener('message', function (event) {
    ...
    if (data.type === 'stellaflix-sonic-workshop-ready') {
      state.ready = true;
      pushProperties(true);
      pushMedia(true);
      pushAudio(true);
  ...
  global.StellaflixSonicWorkshop = {
    INDEX: INDEX,
    isActive: isActive,
    update: update,
    clear: clear,
