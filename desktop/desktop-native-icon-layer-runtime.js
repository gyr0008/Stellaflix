...
function nativeIconLayerGuardCSharpSource() {
  return String.raw`
public static class StellaflixDesktopNativeIconLayerGuard {
  ...
  private static string LayoutKey(StellaflixDesktopIconShapeNative.ProbeResult result) {
    System.Text.StringBuilder key = new System.Text.StringBuilder();
    ...
    if (result.icons != null) {
      foreach (StellaflixDesktopIconShapeNative.IconRect icon in result.icons) {
  ...
  private static void Emit(StellaflixDesktopIconShapeNative.ProbeResult result, bool force) {
    ...
    if (result.icons != null) {
      for (int index = 0; index < result.icons.Length; index++) {
        ...
        StellaflixDesktopIconShapeNative.IconRect icon = result.icons[index];
  ...
  private static void ApplyAndEmit(bool force) {
    ...
    StellaflixDesktopIconShapeNative.ProbeResult result = StellaflixDesktopIconShapeNative.Probe();
  ...
  public static void Run(int debounceMs, int rebindMs, int ownerProcessId,
    ...
    inputThread.Name = "Stellaflix native desktop icon layer input";
    inputThread.Start();
...
[StellaflixDesktopNativeIconLayerGuard]::Run(${debounceMs}, ${rebindMs}, ${ownerProcessId}, [Int64]${iconHostWindowId}, [Int64]${listViewWindowId}, [Int64]${mainWindowId}, ${bounds.x}, ${bounds.y}, ${bounds.width}, ${bounds.height}, ${inputExpression}, ${outputExpression})
`;
}

function nativeIconLayerNamedPipeScript(options = {}) {
  const rawBody = nativeIconLayerGuardScript({ ...options, namedPipeIo: true });
  const invocationMarker = '\n[StellaflixDesktopNativeIconLayerGuard]::Run(';
...
function externalGuardTransport(options = {}) {
  ...
  const pipeBase = `StellaflixNativeIconLayer-${process.pid}-${Date.now()}-${Math.random().toString(16).slice(2)}`;
...
function startNativeDesktopIconLayer(options = {}) {
  ...
  const scriptPath = path.join(
    scriptDirectory,
    `stellaflix-native-icon-layer-${process.pid}-${Date.now()}-${Math.random().toString(16).slice(2)}.ps1`
  );
