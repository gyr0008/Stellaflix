...
const SYSTEM_PURGE_AVAILABLE = isWin && process.env.STELLAFLIX_DISABLE_SYSTEM_MEMORY_PURGE !== '1';
const SYSTEM_PURGE_ENABLED = SYSTEM_PURGE_AVAILABLE && process.env.STELLAFLIX_DISABLE_AUTOMATIC_SYSTEM_MEMORY_PURGE !== '1';
...
const NATIVE_TYPE_BLOCK = [
  ...
  '}',
  'public static class StellaflixMemNative {',
...
function defaultNativeTempPath() {
  const configured = String(process.env.STELLAFLIX_NATIVE_TEMP_DIR || '').trim();
  if (configured) return path.resolve(configured);
  const localRoot = String(process.env.LOCALAPPDATA || process.env.APPDATA || os.tmpdir()).trim();
  return path.join(localRoot, 'Stellaflix', 'native-helper-temp');
}
...
function makeTempPath(label, ext) {
  return path.join(ensureNativeTempPath(), 'stellaflix-' + label + '-' + process.pid + '-' + Date.now() + '.' + ext);
}
  ...
  return new Promise((resolve, reject) => {
    execFile('powershell.exe', ['-NoProfile', '-NonInteractive', '-ExecutionPolicy', 'Bypass', '-File', scriptPath], {
      ...
      env: {
        ...
        STELLAFLIX_NATIVE_TEMP_DIR: ensureNativeTempPath(),
      },
    }, (error, stdout, stderr) => {
...
function buildPurgeScript(mask, resultPath) {
  ...
  const lines = [
    NATIVE_TYPE_BLOCK,
    '[StellaflixMemNative]::PreparePrivileges() | Out-Null',
    '$mask = ' + m,
    '$before = [StellaflixMemNative]::GetUsedPhys()',
    '$loadBefore = [StellaflixMemNative]::GetMemoryLoad()',
    '$steps = @()',
  ];

  if ((m & MEMORY_MASK.workingSet) !== 0) {
    lines.push('$steps += @{ id="workingSet"; status=[StellaflixMemNative]::PurgeList(' + MEMORY_CMD.emptyWorkingSets + ') }');
    lines.push('$steps += @{ id="systemFileCache"; status=[StellaflixMemNative]::FlushSystemFileCache() }');
  }
  if ((m & MEMORY_MASK.modifiedList) !== 0) {
    lines.push('$steps += @{ id="modifiedList"; status=[StellaflixMemNative]::PurgeList(' + MEMORY_CMD.flushModifiedList + ') }');
  }
  if ((m & MEMORY_MASK.standbyList) !== 0) {
    lines.push('$steps += @{ id="standbyList"; status=[StellaflixMemNative]::PurgeList(' + MEMORY_CMD.purgeStandbyList + ') }');
  }
  if ((m & MEMORY_MASK.standbyLow) !== 0) {
    lines.push('$steps += @{ id="standbyLow"; status=[StellaflixMemNative]::PurgeList(' + MEMORY_CMD.purgeStandbyLow + ') }');
  }

  lines.push('$after = [StellaflixMemNative]::GetUsedPhys()');
  lines.push('$loadAfter = [StellaflixMemNative]::GetMemoryLoad()');
...
function probeProcessElevation() {
  ...
  const scriptPath = writeTempScript('elev-check', [
    NATIVE_TYPE_BLOCK,
    'Write-Output ([StellaflixMemNative]::IsTokenElevated() | ConvertTo-Json -Compress)',
  ]);
...
function purgeSystemMemoryElevated(mask, options) {
  ...
  if (!SYSTEM_PURGE_AVAILABLE || (!SYSTEM_PURGE_ENABLED && options.manual !== true)) {
    return Promise.resolve({
      ...
      disabled: true,
      needAdmin: false,
      message: 'Elevated memory purge is disabled by default; Stellaflix will not open administrator PowerShell windows.',
    });
  }
  if (!isWin) {
    return Promise.resolve({ ok: false, unsupported: true, message: 'System memory purge is Windows-only.' });
...
function queryExtendedMemoryStats() {
  ...
  const scriptPath = writeTempScript('mem-stats', [
    NATIVE_TYPE_BLOCK,
    '$total = [StellaflixMemNative]::GetTotalPhys()',
    '$avail = [StellaflixMemNative]::GetAvailPhys()',
    '$load = [StellaflixMemNative]::GetMemoryLoad()',
...
function trimAppWorkingSets(pids) {
  ...
  const scriptPath = writeTempScript('mem-trim', [
    ...
    'public static class StellaflixTrim {',
    ...
    '\'@',
    '$pids = @(' + pidLiteral + ')',
    '$trimmed = [StellaflixTrim]::TrimMany([int[]]$pids)',
