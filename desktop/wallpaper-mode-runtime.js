...
function normalizeWallpaperState(previous, payload, enabledOverride) {
  ...
  return {
    enabled,
    title: String(Object.prototype.hasOwnProperty.call(source, 'title') ? source.title : current.title || 'Stellaflix').slice(0, 512),
...
if (-not ("StellaflixDesktopWallpaperNative" -as [type])) {
Add-Type @"
using System;
using System.Runtime.InteropServices;
public static class StellaflixDesktopWallpaperNative {
...
try { $previousDpiContext = [StellaflixDesktopWallpaperNative]::SetThreadDpiAwarenessContext([IntPtr]::new([Int64]-4)) } catch { }
$progman = [StellaflixDesktopWallpaperNative]::FindWindowByClass("Progman")
if ($progman -eq [IntPtr]::Zero) { throw "WALLPAPER_PROGMAN_NOT_FOUND" }
$sendResult = [IntPtr]::Zero
[StellaflixDesktopWallpaperNative]::SendMessageTimeout($progman, 0x052C, [IntPtr]::Zero, [IntPtr]::Zero, 0, 1000, [ref]$sendResult) | Out-Null
$script:workerw = [IntPtr]::Zero
$callback = [StellaflixDesktopWallpaperNative+EnumWindowsProc]{
  param([IntPtr]$top, [IntPtr]$state)
  $shellView = [StellaflixDesktopWallpaperNative]::FindWindowExByClass($top, [IntPtr]::Zero, "SHELLDLL_DefView")
  if ($shellView -ne [IntPtr]::Zero) {
    $candidate = [StellaflixDesktopWallpaperNative]::FindWindowExByClass([IntPtr]::Zero, $top, "WorkerW")
    if ($candidate -ne [IntPtr]::Zero) { $script:workerw = $candidate }
  }
  return $true
}
[StellaflixDesktopWallpaperNative]::EnumWindows($callback, [IntPtr]::Zero) | Out-Null
if ($script:workerw -eq [IntPtr]::Zero) { throw "WALLPAPER_WORKERW_NOT_FOUND" }
$target = [IntPtr]::new([Int64]${hwnd})
if (-not [StellaflixDesktopWallpaperNative]::IsWindow($target)) { throw "WALLPAPER_TARGET_NOT_FOUND" }
$GWL_STYLE = -16
$WS_POPUP = [Int64]0x80000000
$WS_CHILD = [Int64]0x40000000
$style = [StellaflixDesktopWallpaperNative]::GetWindowLongPtr($target, $GWL_STYLE).ToInt64()
$childStyle = ($style -band (-bnot $WS_POPUP)) -bor $WS_CHILD
[StellaflixDesktopWallpaperNative]::SetWindowLongPtr($target, $GWL_STYLE, [IntPtr]::new([Int64]$childStyle)) | Out-Null
$verifiedStyle = [StellaflixDesktopWallpaperNative]::GetWindowLongPtr($target, $GWL_STYLE).ToInt64()
if (($verifiedStyle -band $WS_CHILD) -eq 0 -or ($verifiedStyle -band $WS_POPUP) -ne 0) { throw "WALLPAPER_CHILD_STYLE_FAILED" }
[StellaflixDesktopWallpaperNative]::SetParent($target, $script:workerw) | Out-Null
$parent = [StellaflixDesktopWallpaperNative]::GetParent($target)
if ($parent -ne $script:workerw) { throw "WALLPAPER_WORKERW_ATTACH_FAILED" }
$origin = New-Object StellaflixDesktopWallpaperNative+POINT
$origin.X = ${x}
$origin.Y = ${y}
if (-not [StellaflixDesktopWallpaperNative]::ScreenToClient($script:workerw, [ref]$origin)) { throw "WALLPAPER_WORKERW_BOUNDS_FAILED" }
$positioned = [StellaflixDesktopWallpaperNative]::SetWindowPos($target, [IntPtr]::new([Int64]1), $origin.X, $origin.Y, ${width}, ${height}, 0x0030)
if (-not $positioned) { throw "WALLPAPER_WORKERW_POSITION_FAILED" }
$className = New-Object System.Text.StringBuilder 128
[StellaflixDesktopWallpaperNative]::GetClassName($script:workerw, $className, $className.Capacity) | Out-Null
[pscustomobject]@{
  ok = $true
...
if ($previousDpiContext -ne [IntPtr]::Zero) {
  try { [StellaflixDesktopWallpaperNative]::SetThreadDpiAwarenessContext($previousDpiContext) | Out-Null } catch { }
}
`;
}
...
class DesktopWallpaperRuntime {
  ...
  sendState(win = this.window) {
    ...
    win.webContents.send('stellaflix-wallpaper-state', this.state);
    return true;
  }
  ...
  createWindow() {
    ...
    const win = new this.BrowserWindow({
      ...
      skipTaskbar: true,
      show: false,
      title: 'Stellaflix Desktop Wallpaper',
      webPreferences: {
