...
const runtimeArg = process.argv.find(value => value.startsWith('--stellaflix-lx-runtime-id='));
const runtimeId = runtimeArg ? runtimeArg.slice('--stellaflix-lx-runtime-id='.length) : '';
const bootstrap = ipcRenderer.sendSync('stellaflix-lx-bootstrap', { runtimeId });
...
function syncCrypto(operation, args) {
  const result = ipcRenderer.sendSync('stellaflix-lx-crypto', { runtimeId, operation, args });
...
function reportInitError(value) {
  ...
  ipcRenderer.send('stellaflix-lx-init-error', {
    runtimeId,
...
contextBridge.exposeInMainWorld('lx', {
  ...
  request(url, options = {}, callback) {
    ...
    ipcRenderer.invoke('stellaflix-lx-http', { runtimeId, requestId, url, options })
      .then(result => {
    ...
    return () => ipcRenderer.send('stellaflix-lx-http-cancel', { runtimeId, requestId });
  },
  ...
  send(eventName, data) {
    if (eventName === EVENT_NAMES.inited) {
      ...
      initialized = true;
      return ipcRenderer.invoke('stellaflix-lx-inited', { runtimeId, data });
    }
    if (eventName === EVENT_NAMES.updateAlert) {
      ...
      return ipcRenderer.invoke('stellaflix-lx-update-alert', { runtimeId, data });
    }
    return Promise.reject(new Error(`The event is not supported: ${eventName}`));
  },
  utils: {
    ...
    zlib: {
      inflate: data => ipcRenderer.invoke('stellaflix-lx-zlib', { runtimeId, operation: 'inflate', data }),
      deflate: data => ipcRenderer.invoke('stellaflix-lx-zlib', { runtimeId, operation: 'deflate', data }),
    },
  },
});

ipcRenderer.on('stellaflix-lx-request', (_event, payload) => {
  if (!payload || !requestHandler) {
    ipcRenderer.send('stellaflix-lx-response', {
      runtimeId,
  ...
  Promise.resolve()
    ...
    .then(result => ipcRenderer.send('stellaflix-lx-response', {
      runtimeId,
      requestKey: payload.requestKey,
      result,
    }))
    .catch(error => ipcRenderer.send('stellaflix-lx-response', {
      runtimeId,
      requestKey: payload.requestKey,
...
ipcRenderer.once('stellaflix-lx-script', (_event, payload) => {
