...
function desktopIconProbeScript(options = {}) {
  ...
  const invocation = options.invoke === false
    ? ''
    : '[StellaflixDesktopIconShapeNative]::Probe() | ConvertTo-Json -Compress -Depth 5';
  return `
$ErrorActionPreference = "Stop"
if (-not ("StellaflixDesktopIconShapeNative" -as [type])) {
Add-Type @"
using System;
using System.Collections.Generic;
using System.Runtime.InteropServices;

public static class StellaflixDesktopIconShapeNative {
...
function desktopIconWatcherCSharpSource() {
  return `
public static class StellaflixDesktopIconShapeWatcherNative {
  ...
  private static bool FindTarget(out IntPtr topLevelHost, out IntPtr iconHost, out IntPtr listView,
    ...
    StellaflixDesktopIconShapeNative.FindDesktopListView(out topLevelHost, out iconHost, out listView);
    processId = 0;
    threadId = 0;
  ...
  private static void RebindIfNeeded(bool forceSnapshot) {
    ...
    if (changed || forceSnapshot) {
      try { Emit(StellaflixDesktopIconShapeNative.Probe(), changed || forceSnapshot); }
  ...
  private static void EmitPendingLayout() {
    ...
    if (!ready) return;
    try { Emit(StellaflixDesktopIconShapeNative.Probe(), false); }
    catch { EmitError("DESKTOP_ICON_PROBE_FAILED"); }
  }
  ...
  public static void Run(int debounceMs, int rebindMs) {
    ...
    inputThread.Name = "Stellaflix desktop icon watcher input";
    inputThread.Start();
...
[StellaflixDesktopIconShapeWatcherNative]::Run(${debounceMs}, ${rebindMs})
`;
}
