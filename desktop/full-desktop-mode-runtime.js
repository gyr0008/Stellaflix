...
if (-not ("StellaflixFullDesktopNative" -as [type])) {
Add-Type @"
using System;
using System.Runtime.InteropServices;
public static class StellaflixFullDesktopNative {
...
try {
  try { $previousDpiContext = [StellaflixFullDesktopNative]::SetThreadDpiAwarenessContext([IntPtr]::new([Int64]-4)) } catch { }
  $target = [IntPtr]::new([Int64]${hwnd})
  if (-not [StellaflixFullDesktopNative]::IsWindow($target)) { throw "FULL_DESKTOP_TARGET_NOT_FOUND" }
  [StellaflixFullDesktopNative]::SetParent($target, [IntPtr]::Zero) | Out-Null
  $GWL_STYLE = -16
  $WS_POPUP = [Int64]0x80000000
  $WS_CHILD = [Int64]0x40000000
  $style = [StellaflixFullDesktopNative]::GetWindowLongPtr($target, $GWL_STYLE).ToInt64()
  $topLevelStyle = ($style -band (-bnot $WS_CHILD)) -bor $WS_POPUP
  [StellaflixFullDesktopNative]::SetWindowLongPtr($target, $GWL_STYLE, [IntPtr]::new([Int64]$topLevelStyle)) | Out-Null
  $verifiedStyle = [StellaflixFullDesktopNative]::GetWindowLongPtr($target, $GWL_STYLE).ToInt64()
  ...
  $parent = [StellaflixFullDesktopNative]::GetParent($target)
  if ($parent -ne [IntPtr]::Zero) { throw "FULL_DESKTOP_DETACH_FAILED" }
  if (-not [StellaflixFullDesktopNative]::SetWindowPos($target, [IntPtr]::Zero, ${x}, ${y}, ${width}, ${height}, 0x0030)) { throw "FULL_DESKTOP_POSITION_FAILED" }
  $rect = New-Object StellaflixFullDesktopNative+RECT
  if (-not [StellaflixFullDesktopNative]::GetWindowRect($target, [ref]$rect)) { throw "FULL_DESKTOP_BOUNDS_ACK_FAILED" }
  ...
  if ($previousDpiContext -ne [IntPtr]::Zero) {
    try { [StellaflixFullDesktopNative]::SetThreadDpiAwarenessContext($previousDpiContext) | Out-Null } catch { }
  }
}
`;
}
...
function desktopWindowCoexistAttachScript(input = {}) {
  ...
  return `
$ErrorActionPreference = "Stop"
if (-not ("StellaflixDesktopCoexistNative" -as [type])) {
Add-Type @"
using System;
using System.Runtime.InteropServices;
using System.Text;
public static class StellaflixDesktopCoexistNative {
...
$previousDpiContext = [IntPtr]::Zero
try {
  try { $previousDpiContext = [StellaflixDesktopCoexistNative]::SetThreadDpiAwarenessContext([IntPtr]::new([Int64]-4)) } catch { }
  $target = [IntPtr]::new([Int64]${hwnd})
  if (-not [StellaflixDesktopCoexistNative]::IsWindow($target)) { throw "FULL_DESKTOP_TARGET_NOT_FOUND" }
  $iconHost = [IntPtr]::Zero
  $defView = [IntPtr]::Zero
  $listView = [IntPtr]::Zero
  $hostClass = ""
  if (-not [StellaflixDesktopCoexistNative]::FindDesktopIconHost([ref]$iconHost, [ref]$defView, [ref]$listView, [ref]$hostClass)) {
    throw "FULL_DESKTOP_ICON_HOST_NOT_FOUND"
  }
  $parentRect = New-Object StellaflixDesktopCoexistNative+RECT
  if (-not [StellaflixDesktopCoexistNative]::GetWindowRect($defView, [ref]$parentRect)) { throw "FULL_DESKTOP_ICON_HOST_BOUNDS_FAILED" }
  $GWL_STYLE = -16
  $WS_POPUP = [Int64]0x80000000
  $WS_CHILD = [Int64]0x40000000
  $style = [StellaflixDesktopCoexistNative]::GetWindowLongPtr($target, $GWL_STYLE).ToInt64()
  $childStyle = ($style -band (-bnot $WS_POPUP)) -bor $WS_CHILD
  [StellaflixDesktopCoexistNative]::SetWindowLongPtr($target, $GWL_STYLE, [IntPtr]::new([Int64]$childStyle)) | Out-Null
  [StellaflixDesktopCoexistNative]::SetParent($target, $defView) | Out-Null
  $verifiedStyle = [StellaflixDesktopCoexistNative]::GetWindowLongPtr($target, $GWL_STYLE).ToInt64()
  if (($verifiedStyle -band $WS_CHILD) -eq 0 -or ($verifiedStyle -band $WS_POPUP) -ne 0) { throw "FULL_DESKTOP_ICON_HOST_STYLE_FAILED" }
  if ([StellaflixDesktopCoexistNative]::GetParent($target) -ne $defView) { throw "FULL_DESKTOP_ICON_HOST_ATTACH_FAILED" }
  $localX = ${x} - $parentRect.Left
  $localY = ${y} - $parentRect.Top
  # Keep the one complete Stellaflix surface below Explorer's real SysListView32.
  # The native icon layer makes Explorer's black background transparent while
  # retaining the real icon pixels and their native hit testing above Stellaflix.
  if (-not [StellaflixDesktopCoexistNative]::SetWindowPos($target, [IntPtr]::new(1), $localX, $localY, ${width}, ${height}, 0x0030)) {
    throw "FULL_DESKTOP_ICON_HOST_POSITION_FAILED"
  }
  $rect = New-Object StellaflixDesktopCoexistNative+RECT
  if (-not [StellaflixDesktopCoexistNative]::GetWindowRect($target, [ref]$rect)) { throw "FULL_DESKTOP_ICON_HOST_BOUNDS_ACK_FAILED" }
  $actualWidth = $rect.Right - $rect.Left
  $actualHeight = $rect.Bottom - $rect.Top
  ...
  if ($previousDpiContext -ne [IntPtr]::Zero) {
    try { [StellaflixDesktopCoexistNative]::SetThreadDpiAwarenessContext($previousDpiContext) | Out-Null } catch { }
  }
}
`;
}
...
class FullDesktopModeRuntime {
    ...
    watcher = this.startDesktopIconWatcherImpl({
      ...
      onExit: (details = {}) => {
        ...
        if (shouldRebind) {
          ...
          // Stellaflix HWND. Explorer has confirmed restoration, so keep the
  ...
  applyInteractivePointerRoute(win = this.window, options = {}) {
    ...
    // A locked desktop surface passes normal Stellaflix areas through to
  ...
  prepareWindow(win, bounds, options = {}) {
    ...
    // The visual HWND remains one continuous Stellaflix surface below Explorer's
  ...
  async reconcileInteractiveInternal(reason = 'interactive-reconcile') {
    ...
    try {
      ...
      // WE base alive behind enabled=true plus a hidden Stellaflix HWND.
