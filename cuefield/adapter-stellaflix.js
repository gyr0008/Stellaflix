...
const STELLAFLIX_BEAT_COMBOS = ['', 'downbeat', 'push', 'drop', 'rebound', 'accent'];
...
function normalizeBeatEvent(raw, index, gridStep) {
  if (Array.isArray(raw)) {
    ...
    const combo = STELLAFLIX_BEAT_COMBOS[Math.max(0, Math.round(toNumber(raw[7], 0)))] || '';
    return {
      time,
      index,
...
function normalizeStellaflixBeatMap(track, map, extra = {}) {
