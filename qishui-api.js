...
const QISHUI_UA = 'Stellaflix/2.1.0 (Qishui official OpenAPI bridge)';
...
const QISHUI_PUBLIC_HEADERS = {
  ...
  'User-Agent': 'Stellaflix/2.1.0 (Qishui public catalog bridge)',
};
  ...
  return {
    ...
    message: webSession
      ...
      : (QISHUI_PUBLIC_ENABLED
        ? '请使用抖音 App 扫描 Stellaflix 中的汽水官方二维码；未登录时仅保留公开搜索匹配。'
        : '请使用抖音 App 扫描 Stellaflix 中的汽水官方二维码完成登录。'),
  };
}
...
async function handleQishuiSearch(keywords, limit, cookieText, offset) {
  ...
  return qishuiSearchCache.wrap(cacheKey, 2 * 60 * 1000, async () => {
    ...
    const payload = {
      ...
      count: limit,
      common_params: {
        trigger_name: 'stellaflix_search',
        scene: 'search',
        source: 'stellaflix',
      },
    };
