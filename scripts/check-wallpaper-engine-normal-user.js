...
const outputRoot = path.join('D:\\StellaflixCache\\we-normal-user-qa', runId);
...
async function waitForCdpTarget(timeoutMs = 60000) {
  ...
  throw new Error(`Timed out waiting for Stellaflix CDP on ${port}: ${lastError && lastError.message || 'no page target'}`);
}
...
function listExactSourceWindows() {
  return listWindows('Stellaflix Wallpaper ');
}

function listDwmSurfaceWindows() {
  return listWindows('Stellaflix WE ');
}
...
async function waitForExactSourceWindowCount(expected, timeoutMs = 20000) {
  ...
  throw new Error(`Exact Stellaflix Wallpaper window count stayed ${windows.length}, expected ${expected}`);
}
...
function assertControlGlassGeometry(state, nativeWindows, round) {
  ...
  assert(/url\(["']?#stellaflix-control-glass-filter["']?\)/.test(bar.backdropFilter),
  ...
  const base = nativeWindows.find((item) => item.title === 'Stellaflix WE DWM Surface');
  assert(base && base.rect && base.visible, `round ${round}: base DWM window is missing or hidden`);
  assert(!nativeWindows.some((item) => item.title === 'Stellaflix WE Glass Refraction'),
...
function assertNativeWindows(state, round) {
  ...
  assert.strictEqual(exactWindows[0].title, `Stellaflix Wallpaper ${state.sessionId}`,
  ...
  const baseWindows = dwmWindows.filter((item) => item.title === 'Stellaflix WE DWM Surface');
  const glassWindows = dwmWindows.filter((item) => item.title === 'Stellaflix WE Glass Refraction');
