...
const outputDirectory = path.resolve(outputArgument
  ...
  : path.join(os.tmpdir(), `stellaflix-we-pointer-target-${Date.now()}`));
...
$expectedTitle = [Environment]::GetEnvironmentVariable('STELLAFLIX_WE_DIAG_SOURCE_TITLE', 'Process')
$targetMode = [Environment]::GetEnvironmentVariable('STELLAFLIX_WE_DIAG_TARGET', 'Process')
$xUnit = [int][Environment]::GetEnvironmentVariable('STELLAFLIX_WE_DIAG_X', 'Process')
$yUnit = [int][Environment]::GetEnvironmentVariable('STELLAFLIX_WE_DIAG_Y', 'Process')
$source = @'
using System;
...
public static class StellaflixWePointerTargetProbe {
...
[StellaflixWePointerTargetProbe]::Run($expectedTitle, $targetMode, $xUnit, $yUnit) | ConvertTo-Json -Compress
`.trim();
}
...
function postPointer(target, xUnit, yUnit) {
  const stdout = execFileSync('powershell.exe', [
    ...
    env: {
      ...process.env,
      STELLAFLIX_WE_DIAG_SOURCE_TITLE: sourceTitle,
      STELLAFLIX_WE_DIAG_TARGET: target,
      STELLAFLIX_WE_DIAG_X: String(xUnit),
      STELLAFLIX_WE_DIAG_Y: String(yUnit),
    },
  });
  return JSON.parse(String(stdout || '').trim());
}

async function captureStellaflix(label) {
  const sources = await desktopCapturer.getSources({
    types: ['window'],
    thumbnailSize: { width: 1440, height: 810 },
    fetchWindowIcons: false,
  });
  const matches = sources.filter((source) => source.name === 'Stellaflix');
  if (matches.length !== 1) {
    throw new Error(`Expected one Stellaflix window capture source, found ${matches.length}`);
  }
...
async function captureAfter(target, side, xUnit) {
  ...
  await wait(settleMs);
  const capture = await captureStellaflix(`${target}-${side}`);
  return { posted, capture };
}

async function run() {
  if (!sourceTitle) throw new Error('Pass --source-title with the exact Stellaflix Wallpaper session title');
  fs.mkdirSync(outputDirectory, { recursive: true });
  const baseline = await captureStellaflix('baseline');
