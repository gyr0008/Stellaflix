...
public sealed class StellaflixDwmThumbnailProbe : Form {
  ...
  public StellaflixDwmThumbnailProbe(IntPtr host, IntPtr source) {
    hostWindow = host;
    sourceWindow = source;
    ...
    Text = "Stellaflix WE DWM Surface Probe";
  ...
  private void FollowHost() {
    ...
    // The probe is immediately behind Stellaflix. The real WE source remains
    // aligned one z-order level further back so it receives the same global
    // cursor geometry without ever covering or receiving Stellaflix clicks.
  ...
  public static void Run(long host, long source) {
    ...
    Application.Run(new StellaflixDwmThumbnailProbe(new IntPtr(host), new IntPtr(source)));
  }
}
'@

[StellaflixDwmThumbnailProbe]::Run($HostWindow, $SourceWindow)
