...
function checkWallpaperEngineImportGuard() {
  ...
  if (!/DwmRegisterThumbnail/.test(dwmSurfaceBlock)
    ...
    || /Stellaflix WE Glass Refraction/.test(dwmSurfaceBlock)
    ...
    || !/source\.name \|\| ''\) === 'Stellaflix WE DWM Surface'/.test(runtimeText)
    || !/stellaflix-wallpaper-engine-glass-surface/.test(mainText)
    || !/kind: 'dwm-glass'/.test(mainText)
    || !/prepareWallpaperEngineRendererGlassCapture/.test(mainText)
    || !/__stellaflixPrepareWallpaperEngineGlassCapture/.test(rendererText)
  ...
  const wallpaperCaptureResultBlock = mainText.slice(
    mainText.indexOf("ipcMain.handle('stellaflix-wallpaper-engine-capture-result'"),
    mainText.indexOf("ipcMain.handle('stellaflix-wallpaper-engine-stop-scene'")
  );
  ...
  if (!/STELLAFLIX_NATIVE_TEMP_DIR/.test(systemMemoryText) || !/setNativeTempPath/.test(systemMemoryText) || !/TEMP:\s*ensureNativeTempPath\(\)/.test(systemMemoryText) || !/NATIVE_HELPER_TEMP_PATH/.test(mainText) || !/nativeTempPath:\s*NATIVE_HELPER_TEMP_PATH/.test(mainText) || !/TEMP:\s*this\.nativeTempPath/.test(runtimeText)) {
    fail('PowerShell Add-Type helpers must use the app-owned stable native temp directory');
  }
  if (/STELLAFLIX_WE_QA_CLEANUP/.test(mainText) || /taskkill\.exe[\s\S]{0,120}wallpaper(?:32|64)/i.test(mainText) || /x:\s*-16000[\s\S]{0,80}y:\s*-16000/.test(mainText) || !/width:\s*Math\.max\(640,[\s\S]{0,220}physicalBounds\.width[\s\S]{0,220}x:\s*physicalBounds\.x,[\s\S]{0,80}y:\s*physicalBounds\.y/.test(mainText)) {
  ...
  if (!/appQuitCleanupPromise/.test(mainText) || !/event\.preventDefault\(\)/.test(mainText) || !/Promise\.race\(\[runtimeCleanup, timeoutCleanup\]\)/.test(mainText)) {
    fail('Wallpaper Engine shutdown must wait briefly for the Stellaflix-owned source window to close');
  }
...
function checkDesktopWallpaperModeGuard() {
  ...
  if ((mainText.match(/new WallpaperEngineRuntime/g) || []).length !== 1
    ...
    || /Stellaflix WE Glass Refraction/.test(wallpaperEngineRuntimeText)
  ...
  if (!createWallpaperWindowBlock
    ...
    fail('entering full desktop mode must expose the complete Stellaflix HUD without creating or requiring a legacy fallback wallpaper');
  }
  if (!/stellaflix-wallpaper-set-enabled/.test(mainText)
    || !/stellaflix-wallpaper-get-status/.test(mainText)
  ...
  if (!/new FullDesktopModeRuntime/.test(mainText)
    || !/stellaflix-full-desktop-icon-shields/.test(mainText + preloadText)
    ...
    fail('full desktop coexistence must preserve the visible Stellaflix HUD, survive native watcher/DWM retries, and remain below the exactly restored Explorer icon plane');
  }
  if (!/id="desktop-mode-control-dock"/.test(htmlText)
    ...
    || !/stellaflix-full-desktop-set-software-lock/.test(mainText)
    || !/stellaflix-full-desktop-set-icons-visible/.test(mainText)
    || !/stellaflix-full-desktop-request-keyboard-focus/.test(mainText)
    || !/stellaflix-full-desktop-pointer-route/.test(mainText)
  ...
  if (!/fullDesktopModeRuntime\.dispose\('app-before-quit'\)/.test(shutdownBlock)
    ...
    fail('shutdown must detach the complete Stellaflix HUD before disposing the single WE DWM chain, with no legacy backdrop cleanup path');
  }
  ...
  console.log('[OK] Legacy canvas backdrop is forbidden; the complete Stellaflix HUD, recoverable software lock, desktop-icon switch, click-outside close, Escape exit, native watcher recovery, single WE DWM layering, and ordered cleanup are guarded.');
}
...
function checkPersistentCacheStorageGuard() {
  ...
  if (!/const CACHE_SETTINGS_FILE/.test(mainText) || !/const LYRIC_CACHE_MAX_BYTES = 96 \* 1024 \* 1024/.test(mainText) || !/function defaultCacheRootPath\(\)/.test(mainText) || !/path\.join\(dDrive, 'StellaflixCache'\)/.test(mainText) || setNameAt < 0 || firstUserDataLookupAt < 0 || setNameAt > firstUserDataLookupAt || !/const STABLE_USER_DATA_PATH = STARTUP_QA_USER_DATA_PATH \|\| path\.join\(app\.getPath\('appData'\), APP_NAME\)/.test(mainText) || !/app\.setPath\('userData', STABLE_USER_DATA_PATH\)/.test(mainText) || !/app\.setPath\('sessionData', chromiumSessionDataPath\(cacheSettings\)\)/.test(mainText) || !/const currentChromiumPath = app\.getPath\('sessionData'\)/.test(mainText) || !/STELLAFLIX_BEAT_CACHE_DIR = cacheSettings\.beatmapsPath/.test(mainText) || !/nativePath:\s*path\.join\(rootPath, 'native-helper-temp'\)/.test(mainText) || !/const NATIVE_HELPER_TEMP_PATH = INITIAL_CACHE_SETTINGS\.nativePath/.test(mainText) || !/activeWallpaperEnginePath/.test(mainText) || !/wallpaperEngineBytes/.test(mainText)) {
  ...
  if (!/stellaflix-cache-get-settings/.test(mainText) || !/stellaflix-cache-set-settings/.test(mainText) || !/stellaflix-cache-read-lyric/.test(mainText) || !/stellaflix-cache-write-lyric/.test(mainText) || !/crypto\.createHash\('sha256'\)/.test(mainText) || !/pruneLyricCache/.test(mainText)) {
...
function checkExternalUpdatePageBridgeGuard() {
  ...
  if (
    !/ipcMain\.handle\('stellaflix-open-update-page', async \(event, value\) =>/.test(mainText)
    ...
    || !/openUpdatePage: \(url\) => ipcRenderer\.invoke\('stellaflix-open-update-page'/.test(preloadText)
  ) {
    fail('desktop update bridge must open only bounded HTTPS pages from the trusted main document');
  }
  if (/stellaflix-open-update-installer|openUpdateInstaller|getUpdateDownloadDir|STELLAFLIX_UPDATE_DIR/.test(bridgeText)) {
    fail('desktop update bridge must not expose the removed local installer or update-cache path');
  }
  if (
    !/stellaflix-download-page/.test(serverText)
...
function checkQishuiProviderGuard() {
  ...
  if (!/search: tokenConfigured \|\| webSession \|\| QISHUI_PUBLIC_ENABLED/.test(qishuiText) || !/loggedIn: webSession/.test(qishuiText) || !/请使用抖音 App 扫描 Stellaflix 中的汽水官方二维码/.test(qishuiText)) {
  ...
  if (!/persist:stellaflix-qishui-auth-v6/.test(qishuiPassportText) || !/a_bogus/.test(qishuiPassportText) || !/check_qrconnect/.test(qishuiPassportText) || !/secondVerify/.test(qishuiPassportText) || !/createQishuiQrLoginBridge/.test(qishuiQrBridgeText)) {
...
function checkPlaybackControlBadgesGuard() {
  ...
  const accountPillGlassSurfaceOk =
    ...
    /html\.control-glass-svg-ok\s+\.top-account-pill::before\s*\{[\s\S]*?url\(#stellaflix-account-pill-glass-filter\)/.test(cssText);
  const accountPillDirectSvgFilter =
    /html\.control-glass-svg-ok\s+\.top-account-pill\s*\{[\s\S]*?url\(#stellaflix-account-pill-glass-filter\)/.test(cssText) ||
  ...
  const accountFilterText = (indexText.match(/<filter id="stellaflix-account-pill-glass-filter"[\s\S]*?<\/filter>/) || [''])[0];
...
async function checkProviderFallbackTerminalStateGuard() {
  ...
  if (!/async function skipFailedQueueItem/.test(fallbackText) || !/skipShuffleOrder:\s*true/.test(fallbackText) || !/return nextStarted === true/.test(fallbackText) || !/function settleSourceFallbackTerminal/.test(fallbackText) || !/audio\.removeAttribute\('src'\)/.test(fallbackText) || !/audio\.__stellaflixQueueItemKey = ''/.test(fallbackText)) {
...
function checkSearchGlassEntranceGuard() {
  ...
  const searchPillDirectSvg = /html\.control-glass-svg-ok\s+\.search-mode-tabs button,[ \t]*\r?\nhtml\.control-glass-svg-ok\s+\.search-history-chip\s*\{[\s\S]{0,260}backdrop-filter:\s*url\(#stellaflix-search-pill-glass-filter\)\s+saturate\(1\)\s*!important[\s\S]{0,220}-webkit-backdrop-filter:\s*url\(#stellaflix-search-pill-glass-filter\)\s+saturate\(1\)\s*!important/.test(cssText);
  ...
  const searchBoxUsesSavedRgbGlassSurface =
    ...
    /html\.control-glass-svg-ok\s+#search-area\.peek\s+#search-box\s*\{[\s\S]{0,180}backdrop-filter:\s*url\(#stellaflix-search-box-glass-filter\)\s+saturate\(1\)\s*!important[\s\S]{0,180}-webkit-backdrop-filter:\s*url\(#stellaflix-search-box-glass-filter\)\s+saturate\(1\)\s*!important/.test(cssText) &&
    ...
    !/html\.control-glass-svg-ok\s+#search-box::before\s*\{[\s\S]{0,260}url\(#stellaflix-search-box-glass-filter\)/.test(cssText) &&
  ...
  const searchPillUsesSavedRgbGlassSurface =
    /html\.control-glass-svg-ok\s+\.search-mode-tabs button,[ \t]*\r?\nhtml\.control-glass-svg-ok\s+\.search-history-chip\s*\{[\s\S]{0,180}background:\s*var\(--saved-button-glass-bg\)\s*!important[\s\S]{0,180}border-color:\s*transparent\s*!important[\s\S]{0,180}box-shadow:\s*var\(--saved-button-glass-shadow\)\s*!important[\s\S]{0,220}url\(#stellaflix-search-pill-glass-filter\)/.test(cssText);
  ...
  const searchBoxFilterText = (indexText.match(/<filter id="stellaflix-search-box-glass-filter"[\s\S]*?<\/filter>/) || [''])[0];
  const searchPillFilterText = (indexText.match(/<filter id="stellaflix-search-pill-glass-filter"[\s\S]*?<\/filter>/) || [''])[0];
  ...
  const searchGlassUsesSavedRgbMapOk =
    ...
    !/stellaflix-search-box-glass-filter|stellaflix-search-pill-glass-filter/.test(chromaticOffsetFunctionText) &&
...
function checkPlaybackResumeRecoveryGuard() {
  ...
  if (
    ...
    || !/__stellaflixTrackSwitchToken\) !== Number\(trackSwitchToken\)/.test(progressText)
...
function checkCuefieldAutoMixGuard() {
  ...
  const adapterText = fs.readFileSync(path.join(appRoot, 'cuefield', 'adapter-stellaflix.js'), 'utf8');
  const bridgeText = fs.readFileSync(path.join(appRoot, 'cuefield', 'stellaflix-bridge.js'), 'utf8');
  ...
  if (!/STELLAFLIX_BEAT_COMBOS/.test(adapterText) || !/raw\[7\]/.test(adapterText) || !/flags & 1/.test(adapterText) || !/flags & 2/.test(adapterText) || !/flags & 4/.test(adapterText) || /raw\[8\][^\n]{0,80}downbeat|raw\[8\][^\n]{0,80}>=\s*7/.test(adapterText)) {
  ...
  if (!/function recoverCuefieldAutoMixEndedOutgoing/.test(integrationText) || !/__stellaflixCuefieldEndedRecoveryToken/.test(playbackText)) {
    fail('Cuefield must resume ordinary queue advance if its outgoing deck ends before a failed handoff settles');
  }
  if (!/__stellaflixPreparedGraphFailed/.test(integrationText) || !/The first element is permanently tied/.test(integrationText)) {
  ...
  if (!/audioSourceMedia = null/.test(coreStoreText) || !/function cuefieldCreatePreparedAudioGraph/.test(integrationText) || !/createMediaElementSource\(media\)/.test(integrationText) || !/__stellaflixPreparedAudioGraph/.test(audioGraphText) || !/preparedGraph\.adopted = true/.test(audioGraphText) || !/audioSourceMedia = audio/.test(audioGraphText)) {
...
function checkInternalBetaPackagingGuard() {
  ...
  const stellaflix = meta.stellaflix || {};
  const update = stellaflix.update || {};
  if (meta.version !== '1.1.2' || beta.productName !== 'Stellaflix_Beat' || meta.productName !== 'Stellaflix_Beat') {
    fail('internal beta package metadata must identify v1.1.2 Stellaflix_Beat');
  }
  ...
  if ((beta.appId || '') !== 'com.stellaflix.beat.internal' || (stellaflix.appUserModelId || '') !== 'com.stellaflix.beat.internal') {
    fail('internal beta must use an isolated app id/AppUserModelID');
  }
  if (stellaflix.runtimeName !== 'Stellaflix_Beat' || update.disabled !== true || update.provider !== 'none') {
  ...
  if (!beta.nsis || beta.nsis.include !== 'build/installer-internal-beta.nsh' || !/Stellaflix_Beat-v\$\{version\}-灰度内测版/.test(beta.nsis.artifactName || '')) {
  ...
  if (!/STELLAFLIX_INSTALL_DIR_NAME "Stellaflix_Beat"/.test(wrapperText) || !/禁止传播/.test(wrapperText) || !/installer\.nsh/.test(wrapperText)) {
    fail('internal beta NSIS wrapper must define Stellaflix_Beat and the no-redistribution notice');
  }
  const installerText = fs.readFileSync(path.join(appRoot, 'build', 'installer.nsh'), 'utf8');
  if (!/STELLAFLIX_INSTALL_DIR_NAME/.test(installerText) || !/STELLAFLIX_INSTALL_NOTICE/.test(installerText)) {
...
function checkSonicTopographyPresetGuard() {
  ...
  const sonicWorkshopBridgeText = fs.readFileSync(path.join(appRoot, 'public', 'vendor', 'sonic-workshop', 'stellaflix-bridge.html'), 'utf8');
  const paletteText = fs.readFileSync(path.join(appRoot, 'public', 'js', 'modules', '02-visual', '07-lyrics-palette-text-utils.js'), 'utf8');
  ...
  if (!/sonic-topography-preset\.js/.test(loaderText) || !/var INDEX = 7/.test(sonicText) || !/function deriveTerrainGridSettings/.test(sonicText) || !/TERRAIN_BASE_SIZE = 168/.test(sonicText) || !/TERRAIN_MAX_GRID_SIZE = 224/.test(sonicText)) {
    fail('Sonic Topography preset must load as a bounded Stellaflix-native port of the latest GitHub visual layer');
  }
  ...
  if (!/function deriveGroundLayoutSettings/.test(sonicText) || !/sonicGroundRange/.test(sonicText) || !/state\.root\.rotation\.x\s*=\s*state\.boundRotX/.test(sonicText) || !/state\.root\.position\.set\(0,\s*layout\.y,\s*layout\.z\)/.test(sonicText) || !/state\.root\.scale\.setScalar\(layout\.scale\)/.test(sonicText)) {
    fail('Sonic Topography must expose a wide, lyric-safe horizontal platter layout inside Stellaflix camera space');
  }
  ...
  if (!/sonic-workshop-preset\.js/.test(loaderText) || !/BRIDGE_SRC = 'vendor\/sonic-workshop\/stellaflix-bridge\.html'/.test(sonicWorkshopText) || !/StellaflixSonicWorkshop\.update/.test(mainLoopText) || !/visual\.sonic-workshop/.test(mainLoopText) || !/StellaflixSonicWorkshop\.onPresetChange/.test(presetGridText) || !/workshopPresetActive/.test(mainLoopText)) {
  ...
  if (!/global\.frequencyData/.test(sonicWorkshopText) || /sonicAudioMonitorState/.test(sonicWorkshopText) || /StellaflixSonicWorkshop\.update\([\s\S]{0,260}audio:\s*sonicAudioFrame/.test(mainLoopText)) {
  ...
  if (/getUserMedia|getDisplayMedia|desktopCapturer|MediaStream|D:\\\\Steam|workshop\\content/.test(sonicWorkshopText + sonicWorkshopBridgeText)) {
    fail('Sonic Workshop derivative must use packaged assets and Stellaflix analyser data instead of runtime capture or the Steam workshop path');
  }
  ...
  if (/getUserMedia|getDisplayMedia|desktopCapturer|MediaStream/.test(sonicAudioText + mainLoopText)) {
    fail('Sonic realtime audio must reuse Stellaflix analyser data instead of requesting system or microphone capture');
  }
  ...
  if (!/StellaflixSonicTopography\.update/.test(mainLoopText) || !/visual\.sonic-topography/.test(mainLoopText) || !/StellaflixSonicTopography\.onPresetChange/.test(presetGridText) || !/StellaflixSonicTopography\.pointerRipple/.test(pointerText)) {
...
function checkPlaylistPanelTriggerGuard() {
  ...
  if (!/--playlist-panel-open-ms:\s*var\(--stellaflix-playlist-panel-open-ms,\s*280ms\)/.test(cssText) || !/--playlist-panel-close-ms:\s*var\(--stellaflix-playlist-panel-close-ms,\s*180ms\)/.test(cssText) || !/setPlaylistPanelCssVar\('--stellaflix-playlist-panel-open-ms'/.test(fxRuntimeText) || !/setPlaylistPanelCssVar\('--stellaflix-playlist-panel-close-ms'/.test(fxRuntimeText)) {
...
const appRoot = process.env.STELLAFLIX_QA_APP_ROOT;
const qaPreload = process.env.STELLAFLIX_QA_PRELOAD;
const pagePath = path.join(appRoot, 'public', 'index.html');
const logs = [];

function finish(code, payload) {
  console.log('STELLAFLIX_QA_RESULT:' + JSON.stringify(payload));
...
app.whenReady().then(async () => {
  ...
  const result = await win.webContents.executeJavaScript(\`
    (async () => {
      ...
      const runtime = typeof window.__stellaflixPerfSnapshot === 'function' ? window.__stellaflixPerfSnapshot() : null;
      const perf = window.__stellaflixPerf && typeof window.__stellaflixPerf.snapshot === 'function'
        ? window.__stellaflixPerf.snapshot()
        : null;
        ...
        try {
          ...
          [2, 3, 4].forEach(tier => {
            ...
            rows.push({
              ...
              markedQuality: !!(built && built.texture && built.texture.userData && built.texture.userData.__stellaflixLyricQuality)
            });
          });
      ...
      async function inspectLyricQualityCacheStability() {
        ...
        try {
          ...
          const immediateSame = lyricQualityCurrentMap(anchor) === oldTexture && !oldTexture.userData.__stellaflixDisposed;
          let sawBase = false;
          ...
          const handoffOk = startedOverNewBudget && immediateSame && !sawBase && anchor.qualityTier === 3 && allTierThree && oldTexture.userData.__stellaflixDisposed && lyricQualityCurrentMap(anchor) === anchor.qualityTexture;
          const newTier = anchor.qualityTier;
          const uploadOk = !window.__stellaflixLyricUploadBudgetStats || Number(window.__stellaflixLyricUploadBudgetStats.maxConsumed) <= 1;
          resetLyricRenderUploadFrameBudget(true);
          updateLyricRowLayers(data, updateOptions);
          ...
          const noDisposeResurrection = data.__stellaflixLyricQualityDisposed === true &&
        ...
        try {
          ...
          for (const target of targets) {
            ...
            while (performance.now() < deadline) {
              ...
              const uploadStats = window.__stellaflixLyricUploadBudgetStats || {};
        ...
        try {
          ...
          const sampleContinuousMotion = () => {
            ...
            if (previousContinuousSample) {
              ...
              const renderFrame = Number(window.__stellaflixLyricUploadBudgetStats && window.__stellaflixLyricUploadBudgetStats.frame) || 0;
            ...
            previousContinuousSample = {
              ...
              visualOffset,
              targetVirtualIndex,
              renderFrame: Number(window.__stellaflixLyricUploadBudgetStats && window.__stellaflixLyricUploadBudgetStats.frame) || 0
            };
      ...
      async function inspectSearchGlassEntrance() {
        ...
        const readSearchBoxGlassStyle = () => {
          ...
          const boxFilter = document.getElementById('stellaflix-search-box-glass-filter');
          const pillFilter = document.getElementById('stellaflix-search-pill-glass-filter');
          const readHref = img => {
            if (!img) return '';
        ...
        const searchBoxFilterLooksLikeSavedRgbGlass = value => String(value || '').includes('stellaflix-search-box-glass-filter') && String(value || '').includes('saturate(1)');
        const searchPillFilterLooksLikeSavedRgbGlass = value => String(value || '').includes('stellaflix-search-pill-glass-filter') && String(value || '').includes('saturate(1)');
...
function runElectronRuntimeCheck() {
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'stellaflix-quick-check-'));
...
try {
  window.localStorage.setItem('stellaflix-startup-fast-skip-v1', 'true');
  window.localStorage.removeItem('stellaflix-cuefield-automix-v1');
} catch (error) {}
`, 'utf8');
  ...
  try {
    const result = spawnSync(electron, [qaScript], {
      cwd: appRoot,
      env: { ...process.env, STELLAFLIX_QA_APP_ROOT: appRoot, STELLAFLIX_QA_PRELOAD: qaPreload },
      encoding: 'utf8',
      timeout: 45000
    });
    if (result.error) {
    ...
    const match = String(result.stdout || '').match(/STELLAFLIX_QA_RESULT:(\{.*\})/);
...
function runMainStartupRecoveryCheck() {
  ...
  const runtimeName = `StellaflixStartupQA-${process.pid}-${Date.now()}`;
  const qaUserDataParent = path.join(process.env.TEMP || appData, 'stellaflix-startup-qa');
  ...
  const qaSessionData = path.join('D:\\StellaflixCache\\chromium', runtimeName);
  try {
    const result = spawnSync(electron, [appRoot], {
      cwd: appRoot,
      env: {
        ...process.env,
        STELLAFLIX_RUNTIME_NAME: runtimeName,
        STELLAFLIX_APP_USER_MODEL_ID: 'com.stellaflix.startup.qa',
        STELLAFLIX_NO_DESKTOP_SHORTCUT: '1',
        STELLAFLIX_STARTUP_QA_USER_DATA: qaUserData,
        STELLAFLIX_STARTUP_TEST_SERVER_DELAY_MS: '4500',
        STELLAFLIX_STARTUP_TEST_FAIL_FIRST_NAV: '1',
        STELLAFLIX_STARTUP_TEST_STALL_LOAD_PROMISE: '1',
        STELLAFLIX_STARTUP_QA_HIDDEN: '1',
        STELLAFLIX_STARTUP_QA_EXIT_MS: '700',
      },
      encoding: 'utf8',
      timeout: 40000,
    });
