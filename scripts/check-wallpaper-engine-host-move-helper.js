...
async function main() {
  assert(Number.isInteger(hostPid) && hostPid > 0, 'A Stellaflix host PID is required');
  const targets = await fetch(`http://127.0.0.1:${port}/json/list`).then((response) => response.json());
  const target = targets.find((item) => item.type === 'page' && /127\.0\.0\.1/.test(item.url || ''));
  assert(target && target.webSocketDebuggerUrl, 'Stellaflix renderer target was not found');
...
$env:TEMP='D:\\StellaflixCache\\native-helper-temp'
$env:TMP=$env:TEMP
Add-Type -TypeDefinition @'
using System;
using System.Runtime.InteropServices;
public static class StellaflixQaHostMove {
...
$rect=New-Object StellaflixQaHostMove+RECT
if (-not [StellaflixQaHostMove]::GetWindowRect($app.MainWindowHandle,[ref]$rect)) { throw 'GetWindowRect failed' }
if (-not [StellaflixQaHostMove]::SetWindowPos($app.MainWindowHandle,[IntPtr]::Zero,$rect.Left+80,$rect.Top+40,0,0,0x0015)) { throw 'SetWindowPos failed' }
`;
  ...
  const moved = spawnSync('powershell.exe', ['-NoLogo', '-NoProfile', '-NonInteractive', '-EncodedCommand', encoded], {
    ...
    env: {
      ...process.env,
      TEMP: 'D:\\StellaflixCache\\native-helper-temp',
      TMP: 'D:\\StellaflixCache\\native-helper-temp',
    },
  });
