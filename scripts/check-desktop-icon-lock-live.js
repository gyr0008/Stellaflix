...
async function mainTarget(timeoutMs = 15000) {
  ...
  throw new Error('Stellaflix CDP target was not ready');
}
...
using System.Text;
public static class StellaflixExplorerUntouchedProbe {
...
$region = [IntPtr]::Zero
try {
  try { $previousDpi = [StellaflixExplorerUntouchedProbe]::SetThreadDpiAwarenessContext([IntPtr]::new([Int64]-4)) } catch { }
  $desktopHost = [IntPtr]::Zero
  $view = [IntPtr]::Zero
  $list = [IntPtr]::Zero
  $expected = [IntPtr]::new([Int64]${expected})
  if ($expected -ne [IntPtr]::Zero) {
    if (-not [StellaflixExplorerUntouchedProbe]::IsWindow($expected)) { throw 'EXPECTED_EXPLORER_LIST_NOT_FOUND' }
    if ([StellaflixExplorerUntouchedProbe]::ClassName($expected) -ne 'SysListView32') { throw 'EXPECTED_EXPLORER_LIST_CLASS_CHANGED' }
    $list = $expected
    $view = [StellaflixExplorerUntouchedProbe]::GetParent($list)
    $desktopHost = [StellaflixExplorerUntouchedProbe]::GetAncestor($list, 2)
  } elseif (-not [StellaflixExplorerUntouchedProbe]::FindDesktop([ref]$desktopHost, [ref]$view, [ref]$list)) {
    throw 'EXPLORER_DESKTOP_LIST_NOT_FOUND'
  }
  if ([StellaflixExplorerUntouchedProbe]::ClassName($list) -ne 'SysListView32') { throw 'EXPLORER_DESKTOP_LIST_INVALID' }
  $rect = New-Object StellaflixExplorerUntouchedProbe+RECT
  if (-not [StellaflixExplorerUntouchedProbe]::GetWindowRect($list, [ref]$rect)) { throw 'EXPLORER_DESKTOP_RECT_FAILED' }
  $region = [StellaflixExplorerUntouchedProbe]::CreateRectRgn(0, 0, 0, 0)
  if ($region -eq [IntPtr]::Zero) { throw 'EXPLORER_DESKTOP_REGION_ALLOC_FAILED' }
  $regionType = [StellaflixExplorerUntouchedProbe]::GetWindowRgn($list, $region)
  $regionBox = New-Object StellaflixExplorerUntouchedProbe+RECT
  $regionBoxType = 0
  if ($regionType -gt 0) { $regionBoxType = [StellaflixExplorerUntouchedProbe]::GetRgnBox($region, [ref]$regionBox) }
  [uint32]$layerColorKey = 0
  [byte]$layerAlpha = 0
  [uint32]$layerFlags = 0
  $layerAvailable = [StellaflixExplorerUntouchedProbe]::GetLayeredWindowAttributes($list, [ref]$layerColorKey, [ref]$layerAlpha, [ref]$layerFlags)
  $layerError = [Runtime.InteropServices.Marshal]::GetLastWin32Error()
  [uint32]$processId = 0
  $threadId = [StellaflixExplorerUntouchedProbe]::GetWindowThreadProcessId($list, [ref]$processId)
  $style = [StellaflixExplorerUntouchedProbe]::GetWindowLongU32($list, -16)
  $exStyle = [StellaflixExplorerUntouchedProbe]::GetWindowLongU32($list, -20)
  $backgroundResult = [IntPtr]::Zero
  if ([StellaflixExplorerUntouchedProbe]::SendMessageTimeout($list, 0x1000, [IntPtr]::Zero, [IntPtr]::Zero, 0x0002, 500, [ref]$backgroundResult) -eq [IntPtr]::Zero) {
    throw 'EXPLORER_DESKTOP_BACKGROUND_TIMEOUT'
  }
  $background = [StellaflixExplorerUntouchedProbe]::IntPtrU32($backgroundResult)
  [pscustomobject]@{
    handle = $list.ToInt64().ToString()
    className = [StellaflixExplorerUntouchedProbe]::ClassName($list)
    parentWindowId = $view.ToInt64().ToString()
    parentClassName = [StellaflixExplorerUntouchedProbe]::ClassName($view)
    rootWindowId = $desktopHost.ToInt64().ToString()
    rootClassName = [StellaflixExplorerUntouchedProbe]::ClassName($desktopHost)
    ...
    enabled = [StellaflixExplorerUntouchedProbe]::IsWindowEnabled($list)
    visible = [StellaflixExplorerUntouchedProbe]::IsWindowVisible($list)
  ...
  if ($region -ne [IntPtr]::Zero) { [StellaflixExplorerUntouchedProbe]::DeleteObject($region) | Out-Null }
  if ($previousDpi -ne [IntPtr]::Zero) { try { [StellaflixExplorerUntouchedProbe]::SetThreadDpiAwarenessContext($previousDpi) | Out-Null } catch { } }
}`;
...
using System.Runtime.InteropServices;
using System.Text;
public static class StellaflixDesktopRouteHitTest {
...
$previousDpi = [IntPtr]::Zero
try {
  try { $previousDpi = [StellaflixDesktopRouteHitTest]::SetThreadDpiAwarenessContext([IntPtr]::new([Int64]-4)) } catch { }
  ...
  for ($depth = 0; $current -ne [IntPtr]::Zero -and $depth -lt 12; $depth++) {
    ...
    [StellaflixDesktopRouteHitTest]::GetClassName($current, $className, $className.Capacity) | Out-Null
    [StellaflixDesktopRouteHitTest]::GetWindowText($current, $title, $title.Capacity) | Out-Null
    $chain += [pscustomobject]@{ handle = $current.ToInt64().ToString(); className = $className.ToString(); title = $title.ToString() }
    $point = New-Object StellaflixDesktopRouteHitTest+POINT
    $point.X = ${Math.round(x)}
    $point.Y = ${Math.round(y)}
    if (-not [StellaflixDesktopRouteHitTest]::ScreenToClient($current, [ref]$point)) { break }
    ...
    $child = [StellaflixDesktopRouteHitTest]::ChildWindowFromPointEx($current, $point, 0x0007)
    if ($child -eq [IntPtr]::Zero -or $child -eq $current) { break }
    $current = $child
  }
  $chain | ConvertTo-Json -Compress
} finally {
  if ($previousDpi -ne [IntPtr]::Zero) { try { [StellaflixDesktopRouteHitTest]::SetThreadDpiAwarenessContext($previousDpi) | Out-Null } catch { } }
}`;
...
async function main() {
  ...
  try {
    ...
    assert.equal(enabled.status && enabled.status.ignoreMouseEvents, false,
      'unlocked complete Stellaflix surface is still globally click-through');
    ...
    assert(enabled.naturalHud.bottomVisible || enabled.naturalHud.homeVisible,
      `full desktop started without a natural Stellaflix HUD: ${JSON.stringify(enabled.naturalHud)}`);
    ...
    assert(iconHit.some((item) => item.className === 'SysListView32'),
      `real Explorer icon did not remain above Stellaflix: ${JSON.stringify(iconHit)}`);
    ...
    assert.equal(naturalInputStatus.ignoreMouseEvents, false,
      'unlocked blank area incorrectly disabled the complete Stellaflix input surface');
    ...
    for (const route of [
      ...
      assert.equal(status.ignoreMouseEvents, false,
        `desktop pointer route unexpectedly disabled Stellaflix input: ${JSON.stringify(route)}`);
    ...
    assert.equal(unlockedBlank.ignoreMouseEvents, false,
      'unlock did not restore natural Stellaflix input outside the controller');
    ...
    assert(persistentHud.bottomVisible || persistentHud.homeVisible,
      `full desktop lost its complete Stellaflix HUD after the ordinary auto-hide path: ${JSON.stringify(persistentHud)}`);
    ...
    console.log(JSON.stringify({
      ...
      manualHardwareChecksStillRequired: [
        ...
        'Lock Stellaflix, move back into the top-right reveal corridor, and use the same switch to unlock it.',
