...
const QA_PARENT = 'D:\\StellaflixCache\\we-lifecycle-qa';
const DEFAULT_EXE = path.resolve(__dirname, '..', '..', '..', 'Stellaflix.exe');
...
const runtimeName = `Stellaflix-WE-Lifecycle-QA-${runId}`;
...
const logPath = path.join(qaRoot, 'stellaflix.log');
...
function ensureQaDirectories() {
  ...
  assert(/^Stellaflix-WE-Lifecycle-QA-[A-Za-z0-9-]+$/.test(path.basename(systemUserDataPath)), 'System QA userData name is not unique and safe');
...
function removeSystemUserDataJunction() {
  ...
  assert(/^Stellaflix-WE-Lifecycle-QA-[A-Za-z0-9-]+$/.test(path.basename(systemUserDataPath)), 'Refusing to remove a non-QA system userData path');
  let stat;
...
public static class StellaflixWeLifecycleWindows {
...
$expected=[Environment]::GetEnvironmentVariable('STELLAFLIX_QA_WINDOW_TITLE')
$items=[Collections.Generic.List[object]]::new()
$callback=[StellaflixWeLifecycleWindows+EnumWindowsProc]{
  param([IntPtr]$handle,[IntPtr]$state)
  $length=[StellaflixWeLifecycleWindows]::GetWindowTextLength($handle)
  if ($length -gt 0) {
    $text=[Text.StringBuilder]::new($length+1)
    [void][StellaflixWeLifecycleWindows]::GetWindowText($handle,$text,$text.Capacity)
    if ($text.ToString() -ceq $expected) {
      [uint32]$ownerPid=0
      [void][StellaflixWeLifecycleWindows]::GetWindowThreadProcessId($handle,[ref]$ownerPid)
      $items.Add([pscustomobject]@{
        handle=$handle.ToInt64().ToString()
        pid=[int]$ownerPid
        visible=[StellaflixWeLifecycleWindows]::IsWindowVisible($handle)
        title=$text.ToString()
      })
    }
  }
...
[void][StellaflixWeLifecycleWindows]::EnumWindows($callback,[IntPtr]::Zero)
ConvertTo-Json -Compress -InputObject @($items)
`, { STELLAFLIX_QA_WINDOW_TITLE: String(title || '') });
  if (!output) return [];
...
function topLevelWindowsByPid(processId) {
  const output = powershellEncoded(`${WINDOW_NATIVE_SOURCE}
$expected=[uint32][Environment]::GetEnvironmentVariable('STELLAFLIX_QA_PROCESS_ID')
$items=[Collections.Generic.List[object]]::new()
$callback=[StellaflixWeLifecycleWindows+EnumWindowsProc]{
  param([IntPtr]$handle,[IntPtr]$state)
  [uint32]$ownerPid=0
  ...
  if ($ownerPid -eq $expected) {
    ...
    if ($length -gt 0) { [void][StellaflixWeLifecycleWindows]::GetWindowText($handle,$text,$text.Capacity) }
    $items.Add([pscustomobject]@{
      handle=$handle.ToInt64().ToString()
      pid=[int]$ownerPid
      visible=[StellaflixWeLifecycleWindows]::IsWindowVisible($handle)
...
[void][StellaflixWeLifecycleWindows]::EnumWindows($callback,[IntPtr]::Zero)
ConvertTo-Json -Compress -InputObject @($items)
`, { STELLAFLIX_QA_PROCESS_ID: String(processId || 0) });
  if (!output) return [];
  const value = JSON.parse(output.split(/\r?\n/).filter(Boolean).slice(-1)[0]);
  return Array.isArray(value) ? value : [value].filter(Boolean);
}
...
function windowHandleExists(handle) {
  const output = powershellEncoded(`${WINDOW_NATIVE_SOURCE}
$handle=[IntPtr]::new([long][Environment]::GetEnvironmentVariable('STELLAFLIX_QA_HOST_HANDLE'))
if ([StellaflixWeLifecycleWindows]::IsWindow($handle)) { '1' } else { '0' }
`, { STELLAFLIX_QA_HOST_HANDLE: String(handle || '') });
...
$handle=[IntPtr]::new([long][Environment]::GetEnvironmentVariable('STELLAFLIX_QA_HOST_HANDLE'))
if (-not [StellaflixWeLifecycleWindows]::IsWindow($handle)) { throw 'QA host window no longer exists' }
$foreground=[StellaflixWeLifecycleWindows]::GetForegroundWindow()
[uint32]$foregroundPid=0
$foregroundThread=if ($foreground -ne [IntPtr]::Zero) { [StellaflixWeLifecycleWindows]::GetWindowThreadProcessId($foreground,[ref]$foregroundPid) } else { 0 }
$currentThread=[StellaflixWeLifecycleWindows]::GetCurrentThreadId()
$attached=$false
if ($foregroundThread -gt 0 -and $foregroundThread -ne $currentThread) {
  $attached=[StellaflixWeLifecycleWindows]::AttachThreadInput($currentThread,$foregroundThread,$true)
}
try {
  [void][StellaflixWeLifecycleWindows]::ShowWindowAsync($handle,9)
  [void][StellaflixWeLifecycleWindows]::SetWindowPos($handle,[IntPtr]::new(-1),0,0,0,0,0x43)
  [void][StellaflixWeLifecycleWindows]::BringWindowToTop($handle)
  [void][StellaflixWeLifecycleWindows]::SetForegroundWindow($handle)
  [void][StellaflixWeLifecycleWindows]::SetWindowPos($handle,[IntPtr]::new(-2),0,0,0,0,0x43)
  Start-Sleep -Milliseconds 120
} finally {
  if ($attached) { [void][StellaflixWeLifecycleWindows]::AttachThreadInput($currentThread,$foregroundThread,$false) }
}
`, { STELLAFLIX_QA_HOST_HANDLE: String(handle || '') });
}
...
$callback=[StellaflixWeLifecycleWindows+EnumWindowsProc]{
  ...
  if ($length -gt 0) {
    ...
    [void][StellaflixWeLifecycleWindows]::GetWindowText($handle,$text,$text.Capacity)
    if ($text.ToString() -ceq $expected) {
      [void][StellaflixWeLifecycleWindows]::PostMessage($handle,0x0010,[IntPtr]::Zero,[IntPtr]::Zero)
    }
  }
  return $true
}
...
async function waitForCdpTarget(port, child, timeoutMs = 60000) {
  ...
  while (Date.now() < deadline) {
    if (child.exitCode != null) throw new Error(`Stellaflix exited before CDP became ready (${child.exitCode})`);
    try {
  ...
  throw new Error(`Timed out waiting for Stellaflix CDP: ${lastError && lastError.message || 'no page target'}`);
}
...
const STATE_EXPRESSION = `(async () => {
  ...
  return {
    ...
    videoTrackSettings,
    capturePath: stream && typeof stream.__stellaflixCapturePath === 'string'
      ? stream.__stellaflixCapturePath
      : '',
    cursorSuppressed: stream && typeof stream.__stellaflixCursorSuppressed === 'boolean'
      ? stream.__stellaflixCursorSuppressed
      : null,
...
async function waitForHostWindow(processId, timeoutMs = 20000) {
  ...
  throw new Error(`QA Stellaflix host window was not found for PID ${processId}: ${JSON.stringify(windows)}`);
}
...
async function main() {
  ...
  assert(fs.existsSync(executablePath), `Stellaflix executable not found: ${executablePath}`);
  ...
  try {
    ...
    const env = {
      ...
      STELLAFLIX_APP_USER_MODEL_ID: `com.stellaflix.we-lifecycle-qa.${runId}`,
      STELLAFLIX_NO_DESKTOP_SHORTCUT: '1',
      STELLAFLIX_CREATE_DESKTOP_SHORTCUT: '0',
      ...(productionThrottling ? {} : { STELLAFLIX_KEEP_BACKGROUND_RENDERING: '1' }),
    };
    ...
    assert(actualUserDataPath && fs.existsSync(actualUserDataPath), 'QA userData path was not reported by Stellaflix');
    ...
    const initialTitle = `Stellaflix Wallpaper ${initial.sessionId}`;
    ...
    const stopAllRaceTitle = `Stellaflix Wallpaper ${afterStopAllRace.sessionId}`;
    ...
    const minimizeRestoreTitle = `Stellaflix Wallpaper ${restoredFromMinimize.sessionId}`;
    ...
    const hideRestoreTitle = `Stellaflix Wallpaper ${restoredFromHide.sessionId}`;
    ...
    const fullscreenTitle = `Stellaflix Wallpaper ${fullscreen.sessionId}`;
    ...
    const fullscreenRestoreTitle = `Stellaflix Wallpaper ${restoredFromFullscreen.sessionId}`;
    ...
    assert(exit, 'QA Stellaflix did not exit within 20 seconds');
    cleanExit = true;
    ...
    assert.strictEqual(windowHandleExists(hostHandle), false, 'QA Stellaflix host window survived process exit');
