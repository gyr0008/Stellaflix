...
async function run() {
  ...
  try {
    const prepared = await client.evaluate(`(async () => {
      ...
      window.__stellaflixKeyboardProbe = { trustedPointerDown: false, pointerTarget: '', f13: 0 };
      document.addEventListener('pointerdown', (event) => {
        window.__stellaflixKeyboardProbe.trustedPointerDown = event.isTrusted === true;
        window.__stellaflixKeyboardProbe.pointerTarget = event.target && event.target.id || '';
      }, { once: true, capture: true });
      window.__stellaflixKeyboardProbeKeyListener = (event) => {
        if (event.code === 'F13') window.__stellaflixKeyboardProbe.f13 += 1;
      };
      document.addEventListener('keydown', window.__stellaflixKeyboardProbeKeyListener, { capture: true });
      return {
        status,
    ...
    const marker = `StellaflixKeyboardFocus-${Date.now()}`;
    ...
    const restored = await client.evaluate(`(() => {
      const input = document.getElementById('search-input');
      const probe = window.__stellaflixKeyboardProbe || {};
      const snapshot = {
        documentFocused: document.hasFocus(),
    ...
    assert.equal(restored.documentFocused, true,
      'trusted Stellaflix pointerdown did not restore renderer keyboard focus');
    ...
    console.log(JSON.stringify({
      ...
      manualHardwareChecksStillRequired: [
        ...
        'After clicking a real desktop icon, click Stellaflix again and test Space, arrows, and configured local hotkeys.',
      ],
    ...
    try {
      await client.evaluate(`(async () => {
        ...
        if (window.__stellaflixKeyboardProbeKeyListener) {
          document.removeEventListener('keydown', window.__stellaflixKeyboardProbeKeyListener, true);
          delete window.__stellaflixKeyboardProbeKeyListener;
        }
        delete window.__stellaflixKeyboardProbe;
