...
async function main() {
  const temp = fs.mkdtempSync(path.join(os.tmpdir(), 'stellaflix-we-'));
  ...
  try {
    ...
    const mediaUrl = (kind, id) => `stellaflix-wallpaper://${kind}/${id}?token=${snapshot.mediaToken}`;
    ...
    const missingToken = await instance.mediaResponse(new Request(`stellaflix-wallpaper://media/${video.id}`));
    assert.strictEqual(missingToken.status, 404);
    const malformed = await instance.mediaResponse(new Request(`stellaflix-wallpaper://media/%E0%A4%A?token=${snapshot.mediaToken}`));
