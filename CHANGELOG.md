...
- 观看历史升级为番剧聚合模型：一部番一条记录，卡片显示「第 N 话」，删除时联动清除各集进度，并兼容 v1 数据迁移。
- 新增 Subsonic/Navidrome 远程音乐库：服务端接口、UI 接入，本地音乐弹窗改为从列表选歌播放。
- 新增片库海报墙与首页收藏浮层，下线旧版合集页。
- 新增 HLS 广告过滤与播放卡顿看门狗。
- 播放器支持跨源续播与剧集线路导航；修复观看历史回写与首页续播返回。
- 新增 Agent 日推与播放意图前缀解析。
- 修复 Wallpaper Engine 原生引擎中央黑屏、本地音乐封面跨源污染等问题。
- 补充海报墙、Subsonic、HLS 广告过滤等回归测试。

## v0.1.2

- 修复应用内更新下载卡在 100% 不进入安装：增加下载/校验看门狗，卡住自动切镜像或降级网盘；100% 后提示「正在校验」。

## v0.1.1

- **修复连点桌面图标导致应用被强制退出**：二次启动不再 `taskkill` 正在运行的主进程；改为聚焦已有窗口后静默退出。

## v0.1.0

- 正式版本号纠正为 0.1.0（此前误写为 2.1.0）。
- 优化 Wallpaper Engine 壁纸与全屏模式的兼容性。
- 改进登录、账号状态和本地曲库体验。
- 提升长时间运行与连续播放稳定性。
- 修复多行歌词与 3D 歌单架的显示层级。
- 优化更新入口：应用内自动更新为主，网盘/Release 外链为辅。
...
- P0 installer safety fix: installation now defaults to the first available non-C drive from `D:\Stellaflix` through `Z:\Stellaflix`; it falls back to `C:\Stellaflix` only when no D-Z drive exists.
- The installer now forces the target path into a dedicated `Stellaflix` folder, blocks non-empty non-Stellaflix-owned targets, and blocks C drive installs when a D-Z drive is available.
...
- Existing dedicated `...\Stellaflix` folders that already contain Stellaflix files can be overwritten even if they were created before the new safety marker existed.
- The new uninstaller no longer removes the whole installation root or app subfolders recursively; it deletes only known top-level Stellaflix/Electron files and leaves unrelated files in place.
...
- 安装包支持用户自由选择安装目录；默认仍优先 `D:\Stellaflix`，选择盘根目录时会自动补全 `Stellaflix` 文件夹。
...
- 发布 Stellaflix 首个正式版本，GitHub 主页和安装包版本统一为 `1.0.0`。
