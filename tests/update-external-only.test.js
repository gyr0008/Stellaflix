...
test('update metadata accepts only a bounded HTTPS external page', () => {
  ...
  assert.match(serverText, /stellaflix-download-page/);
...
test('release body preserves all three labelled HTTPS download pages', () => {
  ...
  const pages = sandbox.extractReleaseDownloadPages([
    '<!-- stellaflix-download-page: 夸克盘|https://pan.quark.cn/s/f40289e1c5d3 -->',
    '<!-- stellaflix-download-page: 百度云|https://pan.baidu.com/s/14fgTABgbfseOg9QuX0Um7Q?pwd=sjhp -->',
    '<!-- stellaflix-download-page: 蓝奏云|https://xxhuber.lanzout.com/stellaflix2 -->',
    '<!-- stellaflix-download-page: 不安全|http://example.com/file -->',
  ].join('\n'));
  assert.deepEqual(JSON.parse(JSON.stringify(pages)), [
    ...
    { label: '蓝奏云', url: 'https://xxhuber.lanzout.com/stellaflix2' },
  ]);
});
