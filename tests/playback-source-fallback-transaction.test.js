...
function testStaticRecoveryWiring() {
  ...
  assert(/audio\.__stellaflixTrackSwitchToken = token/.test(startText));
  assert(/audioEl !== audio/.test(progressText));
  assert(/__stellaflixTrackSwitchToken\) !== Number\(trackSwitchToken\)/.test(progressText));
  assert(/playbackMediaMatchesCurrentQueueItem\(audioEl\)/.test(progressText));
  assert(/ownerQueueItemKey:\s*String\(audioEl\.__stellaflixQueueItemKey/.test(progressText));
