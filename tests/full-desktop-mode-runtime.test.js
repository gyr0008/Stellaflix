...
function makeRuntime(options = {}) {
  ...
  const runtime = new FullDesktopModeRuntime({
    screen,
    platform: 'win32',
    nativeTempPath: 'D:\\StellaflixCache\\native-helper-temp',
...
test('enable defaults to a fully interactive Stellaflix desktop', async () => {
  ...
  assert.equal(calls.coexist.length, 1, 'interactive enable must attach the complete Stellaflix surface below the real desktop icons');
  ...
  assert.deepEqual(win.shape, [], 'continuous Stellaflix surface must not contain desktop icon holes');
});
...
test('passive preparation failure leaves the full Stellaflix window interactive', async () => {
  const win = new FakeBrowserWindow();
  const { runtime, calls } = makeRuntime({
...
test('software lock passes normal areas through while the controller remains an unlock island', async () => {
  ...
  assert.equal(win.ignoreMouse, false, 'unlock from the controller must immediately restore natural Stellaflix input');
...
test('coexist attach places the complete Stellaflix surface below SysListView without mouse synthesis', () => {
...
test('unexpected watcher exit with confirmed restoration queues recovery without hiding Stellaflix', async () => {
  const win = new FakeBrowserWindow();
