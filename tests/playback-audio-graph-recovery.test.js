...
function makeHarness(options = {}) {
  ...
  const audio = makeAudio({
    ...
    __stellaflixQueueItemKey: 'queue-alpha',
  });
...
function testCaptureTrackSwitchCreatesFreshAudioLifetime() {
  ...
  assert.equal(context.source.__stellaflixUsesCapture, false);
  assert.equal(audio.__stellaflixMediaSourceBound, true);
  ...
  context.source.__stellaflixUsesCapture = true;
  context.audioSourceMedia = audio;
  context.audioReady = true;
  assert.equal(context.source.__stellaflixUsesCapture, true, 'test did not establish the legacy capture-backed state');
  ...
  assert.equal(audio.__stellaflixMediaSourceBound, true, 'lifetime MediaElementSource marker was cleared');
  ...
  assert.equal(freshAudio.__stellaflixQueueItemKey, 'queue-alpha', 'fresh Audio lost queue identity');
  ...
  assert.equal(context.initAudio(), true);
  assert.equal(context.source.__stellaflixUsesCapture, false, 'fresh Audio did not establish MediaElementSource');
  ...
  assert.equal(freshAudio.__stellaflixMediaSourceBound, true);
  assert.equal(freshAudio.__stellaflixQueueItemKey, 'queue-alpha');
}
...
function testTwoAdvancingSilentSamplesTriggerCaptureRebuild() {
  ...
  assert.equal(context.source.__stellaflixUsesCapture, false, 'bound media was incorrectly moved to captureStream');
...
function testTransientCaptureSourceFailureRetainsForcedCaptureRetry() {
  ...
  const originalAudio = audio;
  audio.__stellaflixForceCaptureSource = true;
  ...
  assert.equal(audio.__stellaflixForceCaptureSource, true, 'failed capture attempt consumed the forced-capture retry');
  ...
  assert.equal(context.source.__stellaflixUsesCapture, true);
  ...
  assert.strictEqual(context.audio, originalAudio);
  assert.equal(!!audio.__stellaflixMediaSourceBound, false);
  assert.equal(audio.__stellaflixQueueItemKey, 'queue-alpha');
}
...
function testForcedCaptureWaitsForMediaReadyState() {
  ...
  assert.equal(audio.__stellaflixForceCaptureSource, true, 'ready-state wait consumed the forced-capture marker');
  assert.strictEqual(context.audio, originalAudio);
