...
async function run() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'stellaflix-login-gate-'));
  try {
    ...
    assert(main.includes("ipcMain.handle('stellaflix-login-easter-egg-reset'"));
    ...
    assert(preload.includes("resetLoginEasterEgg: () => ipcRenderer.invoke('stellaflix-login-easter-egg-reset')"));
    ...
    assert(desktopMain.includes("ipcMain.handle('stellaflix-full-desktop-request-keyboard-focus'"));
