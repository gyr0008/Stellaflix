...
function testRendererGoneDelayedRecovery() {
  ...
  assert.match(recoveryBlock, /const keepIntentionallyHidden = win\.__stellaflixIntentionalHide === true/, '恢复必须快照托盘主动隐藏状态');
  assert.match(
    recoveryBlock,
    /win\.__stellaflixIntentionalHide = keepIntentionallyHidden;\s*if \(!keepIntentionallyHidden\) showMainWindowSafely\(win, `renderer-recovered-\$\{attempt\}`\);\s*else sendWindowState\(win\)/,
...
function testFullscreenVisibilityAndSystemWakeGuards() {
  ...
  assert.match(visibilityBlock, /win\.__stellaflixIntentionalHide === true/, '托盘主动隐藏必须跳过可见性守护');
  ...
  assert.match(mainText, /win\.__stellaflixIntentionalHide = true;[\s\S]{0,180}win\.hide\(\)/, '托盘隐藏必须先标记为 intentional');
  assert.match(mainText, /win\.on\('show'[\s\S]{0,120}win\.__stellaflixIntentionalHide = false/, '重新显示后必须解除 intentional hide');
