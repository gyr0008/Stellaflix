...
test('startup QA isolates its disposable userData away from the real profile', () => {
  assert.match(mainText, /STELLAFLIX_STARTUP_QA_USER_DATA/);
  assert.match(mainText, /STELLAFLIX_STARTUP_QA_HIDDEN !== '1'/);
  ...
  assert.match(quickCheckText, /path\.join\(process\.env\.TEMP \|\| appData, 'stellaflix-startup-qa'\)/);
  assert.match(quickCheckText, /STELLAFLIX_STARTUP_QA_USER_DATA:\s*qaUserData/);
