...
async function testTrustedReadySignalRequiresTheExpectedDocument() {
  ...
  assert.deepStrictEqual(
    { url: win.__stellaflixTrustedMainDocumentReady.url, phase: win.__stellaflixTrustedMainDocumentReady.phase },
...
async function testStalledLoadPromiseAcceptsTrustedDomReady() {
  const harness = createLoadWithRetryHarness({ STELLAFLIX_STARTUP_TEST_STALL_LOAD_PROMISE: '1' });
...
async function testTrulyUnreadyDocumentStillRetriesAndFails() {
  ...
  await assert.rejects(
    ...
    error => error && error.code === 'STELLAFLIX_STARTUP_TIMEOUT'
  );
...
function testSourceContractKeepsTimeoutFailOpenNarrow() {
  ...
  assert.match(loadWithRetrySource, /STELLAFLIX_STARTUP_TIMEOUT[\s\S]{0,100}readySignal\.isReady\(\)/);
  assert.match(loadWithRetrySource, /finally \{[\s\S]{0,100}readySignal\.cancel\(\)/);
  assert.match(loadWithRetrySource, /STELLAFLIX_STARTUP_TEST_STALL_LOAD_PROMISE/);
}
