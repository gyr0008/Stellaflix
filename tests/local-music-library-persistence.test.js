...
function vorbisCommentBlock(comments) {
  const vendor = Buffer.from('Stellaflix test', 'utf8');
  const parts = [];
...
test('FLAC tags, embedded cover and same-name LRC survive a full library reload', async (t) => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'stellaflix-local-library-'));
  ...
  assert.match(imported.tracks[0].cover, /^stellaflix-local:\/\/cover\/[a-f0-9]{24}/);
  assert.match(imported.tracks[0].localUrl, /^stellaflix-local:\/\/audio\/[a-f0-9]{24}/);
...
test('subsequent imports append to the persistent library by default', async (t) => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'stellaflix-local-library-append-'));
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
...
test('metadata and manifest failures preserve the last committed cover and index', async (t) => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'stellaflix-local-library-transaction-'));
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
...
test('renderer and Electron wiring restore persistent tracks instead of blob-only missing records', () => {
  ...
  assert.doesNotMatch(main, /stellaflix-local-library-read-sync/);
  assert.match(main, /await localMusicLibrary\.listTracks\(\)/);
  assert.match(main, /stellaflix-local-library-lyric/);
  assert.match(main, /stellaflix-local-library-authorize/);
  ...
  assert.match(cover, /stellaflix-local:\\\/\\\/cover/);
